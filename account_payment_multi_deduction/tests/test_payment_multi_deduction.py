...
class TestPaymentMultiDeduction(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.outstanding_receipt = cls.env.ref(
            "account.1_account_journal_payment_debit_account_id"
        )
        cls.outstanding_payment = cls.env.ref(
            "account.1_account_journal_payment_credit_account_id"
        )
        ...
        # Add account in payment method line
        cls.journal_bank.outbound_payment_method_line_ids.payment_account_id = (
            cls.outstanding_receipt.id
        )
        cls.journal_bank.inbound_payment_method_line_ids.payment_account_id = (
            cls.outstanding_payment.id
        )

        cls.cust_invoice = cls.move_model.create(
            {
                "name": "Test Customer Invoice",
                "move_type": "out_invoice",
                "journal_id": cls.journal_model.search(
                    [("type", "=", "sale")], limit=1
                ).id,
    ...
    def test_one_invoice_payment(self):
        ...
        self.assertRecordValues(
            ...
            [
                {
                    "account_id": self.outstanding_payment.id,
                    "name": "Manual Payment: Test Customer Invoice",
                    "debit": 400.0,
                    "credit": 0.0,
                },
                {
    ...
    def test_invoice_payment_fully_paid(self):
        ...
        action = self.cust_invoice.action_register_payment()
