...
class cheque_setting(models.Model):
    ...
    decimal_format_type = fields.Selection(selection=[('default', 'Default'), ('custom', 'Custom')], default='default',
                                           required=True)
    decimal_value = fields.Char(string='Decimal')
    print_only = fields.Boolean(string="Print word 'Only'")
    decimal_example = fields.Text(string='Example', compute='compute_decimal_example')

    @api.depends('decimal_format_type', 'print_only', 'decimal_value')
    def compute_decimal_example(self):
        for rec in self:
            amount = '''Amount : 169.05\n'''
            decimal = '''One Hundred And Sixty-Nine And 05 / 100'''
            if rec.decimal_format_type == 'custom':
                decimal = '''One Hundred And Sixty-Nine And Five %s ''' % (rec.decimal_value)
            if rec.print_only:
                decimal += ''' Only'''
            rec.decimal_example = amount + decimal
        ...
        for line in self:
            if line.set_default:
                line_ids = self.env['cheque.setting'].search(
                    [('set_default', '=', True), ('company_id', '=', line.company_id.id)])
                if len(line_ids) > 1:
