...
# Let's read some invoices including INV-E/26/05/00021
moves, err = call_kw('account.move', 'search_read', [
    [('name', 'in', ['INV-E/26/05/00021', 'INV-D/26/06/00010', 'INV-D/26/06/00006'])],
    ['name', 'invoice_origin', 'invoice_line_ids']
], {})

if moves:
    print("Tracing warehouse with fallback logic:")
    for m in moves:
        warehouse = None
        # 1. Line level Sale Order
        ...
        # 2. Fallback to Origin matching Sale Order
        if not warehouse and m.get('invoice_origin'):
            so, err = call_kw('sale.order', 'search_read', [
                [('name', '=', m['invoice_origin'])],
                ['warehouse_id']
            ], {})
            if so:
                warehouse = so[0]['warehouse_id']
            else:
                # Fallback to PO
                po, err = call_kw('purchase.order', 'search_read', [
                    [('name', '=', m['invoice_origin'])],
                    ['picking_type_id']
                ], {})
                if po and po[0].get('picking_type_id'):
                    pt_id = po[0]['picking_type_id'][0]
                    pt, err = call_kw('stock.picking.type', 'search_read', [
                        [('id', '=', pt_id)],
                        ['warehouse_id']
                    ], {})
                    if pt:
                        warehouse = pt[0]['warehouse_id']
        
        print(f"Invoice: {m['name']} | Origin: {m['invoice_origin']} | Warehouse: {warehouse}")
else:
