...
class AccountPayment(models.Model):
    ...
    def _is_bank_draft_payment_method(self):
        self.ensure_one()
    ...
    def _get_instrument_type(self):
        self.ensure_one()
        return "bank_draft" if self._is_bank_draft_payment_method() else "cheque"

    def _get_instrument_payment_account(self):
        self.ensure_one()
        ...
        candidate = method_lines.filtered(
            lambda line: line.payment_account_id
            and (
                line.is_cheque_incoming_line
                or line.is_cheque_outgoing_line
                or line.is_bank_draft_incoming_line
                or line.is_bank_draft_outgoing_line
                or line.code in ("cheque", "cheque_incoming", "cheque_outgoing", "bank_draft")
            )
        )[:1]
        return candidate.payment_account_id
    ...
    def _prepare_move_line_cheque_detail(self):
        ...
        if self.outbound_cheque_lines:
            for line in self.outbound_cheque_lines:
                payment_account = self._get_instrument_payment_account()
    ...
    def _get_receivable_payable_line_vals_hook(self, receivable_payable_line_vals):
        ...
        if (self.outbound_cheque_lines or self.inbound_cheque_lines) and line_vals_list:
            account = False
            if self.payment_type == "outbound":
                cheque_lines = self.journal_id.outbound_payment_method_line_ids.filtered(
                    lambda l: l.is_cheque_outgoing_line
                    and l.payment_account_id
                    and l.payment_account_id.account_type == "liability_payable"
                )
                if len(cheque_lines) > 1:
                    raise UserError(
                        _(
                            "Only one outgoing cheque payment method line is allowed with a payable account."
                        )
                    )
                account = cheque_lines.payment_account_id
            if not account:
                account = self._get_instrument_payment_account()
            if account:
    ...
    def _cheque_mark_state_after_external_flow(self, target_state, matched=False):
        """Update payment lifecycle state after cheque-side accounting actions.

        Cheque flow may reverse/post journal entries outside account.payment native
        actions. Keep the move entries untouched here and only align payment state.
        """
        vals = {"state": target_state, "is_matched": matched}
        self.write(vals)
        invoices = (self.reconciled_invoice_ids | self.invoice_ids).filtered(
    ...
    def create(self, vals_list):
        ...
        if res.outbound_cheque_lines:
            if not res._get_instrument_payment_account():
                raise UserError(
                    _("Please set Outgoing Cheque Account first from Journal")
                )
            ...
            for line in res.outbound_cheque_lines:
                if not line.is_cheque_created:
                    ...
                    existing_cheque_inbound_outbound_id = self.env[
                        ...
                        ("cheque_type", "=", "inbound"),
                        ("state", "in", ["draft", "waiting_confirm", "confirmed", "bank_deposit"]),
                    ...
                    if existing_cheque_inbound_outbound_id:
                        ...
                        new_cheque_paying = self.env["cheque.inbound.outbound"].create(
                            {
                                ...
                                "instrument_type": res._get_instrument_type(),
                                "cheque_type": "inbound",
                                "payment_method_line_id": payment_method_line_id.id,
                                "payment_method_line_account_id": res._get_instrument_payment_account().id,
                            }
                        )
                        ...
                        new_cheque_paying.action_arrive_confirm()
                        line.cheque_id.status = "confirmed"
            ...
            for line in res.inbound_cheque_lines:
                if not line.is_cheque_created:
                    ...
                    if existing_cheque_inbound_outbound_id:
                        ...
                        new_cheque_receiving.action_arrive_confirm()
                    line.is_cheque_created = True
        res.cheque_inbound_outbound_ids = [(6, 0, cheque_inbound_outbound_list)]
        return res

    def action_post(self):
        # Cheque flow: keep payment draft until Bank Deposit calls action_post with force_post_cheque.
        force_post_cheque = self.env.context.get("force_post_cheque")
        cheque_payments = self.filtered(
            lambda p: (
                p.cheque_inbound_outbound_ids
                or p.outbound_cheque_lines
                or p.inbound_cheque_lines
                or p.payment_method_line_id.is_cheque_incoming_line
                or p.payment_method_line_id.is_cheque_outgoing_line
                or p.payment_method_line_id.is_bank_draft_incoming_line
                or p.payment_method_line_id.is_bank_draft_outgoing_line
                or p.payment_method_line_id.code == "bank_draft"
            )
        )
        normal_payments = self - cheque_payments

        res = True
        if normal_payments:
            res = super(AccountPayment, normal_payments).action_post()

        if cheque_payments and force_post_cheque:
            super(AccountPayment, cheque_payments).action_post()
    ...
    def write(self, vals):
            ...
            if vals.get("inbound_cheque_lines"):
                for line in rec.inbound_cheque_lines:
                    cheque_paying_obj = self.env["cheque.inbound.outbound"].search(
                        [
                            ("state", "in", ["waiting_confirm", "confirmed"]),
        ...
        if vals.get("outbound_cheque_lines"):
            if self.outbound_cheque_lines:
                if not self._get_instrument_payment_account():
                    raise UserError(
                        _("Please set Outgoing Cheque Account first from Journal")
                    )
                        ...
                        if existing_cheque_inbound_outbound_id:
                            ...
                            new_cheque_paying = self.env[
                            ...
                            ].create(
                                {
                                    ...
                                    "cheque_type": "inbound",
                                    "payment_method_line_id": payment_method_line_id.id,
                                    "payment_method_line_account_id": self._get_instrument_payment_account().id,
                                }
                            )
                            cheque_inbound_outbound_list.append(new_cheque_paying.id)
                            self.env["cheque.payment.detail.lines"].create(
...
class InboundChequeLine(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            if "amount" in vals and rec.amount <= 0.0:
                    raise UserError(
                        _(
                            "Amount must be greater than 0 in cheque line %s .",
                            rec.cheque_id,
