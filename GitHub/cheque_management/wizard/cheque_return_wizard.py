...
class ChequeReturnWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        rec['cheque_no'] = cheque_in_out_bound_obj.name
    ...
    def action_confirm_cheque_return(self):
        cheque_in_out_bound_obj = self.env['cheque.inbound.outbound'].browse(self._context.get('active_ids'))
        # For paid/deposited/transformed cheques use the unified reversal-based void flow.
        if any(ch.state in ['paid', 'bank_deposit', 'transform'] for ch in cheque_in_out_bound_obj):
            return self.action_void()
        ...
        if self.cheque_no:
            cheque_in_out_bound_obj.name = self.cheque_no
        if not self.return_account_id:
            raise UserError(_('Please set Outgoing Cheque Account for Returned first from Journal'))

        if self.void_reason:
            body_msg = 'Void Reason: %s.' % self.void_reason
            for cheque in cheque_in_out_bound_obj:
                cheque.message_post(
                    body=body_msg,
        ...
        if cheque_in_out_bound_obj.cheque_type == 'inbound':
            ...
            debit_line = {
                'account_id': cheque_in_out_bound_obj.payment_method_line_id.default_account_id.id,
                'partner_id':  cheque_in_out_bound_obj.pay_partner_id.id,
                'name': 'Return Cheque No ' + str(cheque_in_out_bound_obj.name),
    ...
    def action_void(self):
        ...
        for cheque in cheque_in_out_bound_obj:
            if cheque.state not in ['paid', 'bank_deposit', 'transform']:
                raise UserError(_('Void is only available for Paid or Deposited cheques.'))

            # Use the enhanced action_cancel which handles reversal of all moves (Clearing, Deposit, Payment)
            cheque.action_cancel()
            cheque.void_reason = self.void_reason
            if self.cheque_no:
                cheque.name = self.cheque_no
