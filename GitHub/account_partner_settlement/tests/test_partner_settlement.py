...
class TestPartnerSettlement(TransactionCase):
    ...
    def test_payment_register_customer_credit_note_selected_in_wizard_is_embedded_in_payment_move(self):
        invoice = self._create_customer_invoice(100.0)
        credit_note = self._create_customer_refund(25.0)

        wizard = self._create_settlement_wizard(invoice)
        credit_line = wizard.same_side_credit_note_line_ids.filtered(lambda line: line.move_id == credit_note)
        self.assertTrue(credit_line)

        credit_line.is_selected = True
        wizard._onchange_same_side_credit_note_line_ids()
        wizard._compute_payment_difference()
        wizard._compute_show_payment_difference()

        self.assertAlmostEqual(wizard.same_side_credit_note_amount, 25.0)
        self.assertAlmostEqual(wizard.amount, 75.0)
        self.assertAlmostEqual(wizard.payment_difference, 0.0)
        self.assertFalse(wizard.show_payment_difference)
        self.assertAlmostEqual(sum(wizard.allocation_line_ids.mapped("amount_to_pay")), 100.0)
        ...
        self.assertTrue(payment.same_side_credit_note_embedded)
        self.assertAlmostEqual(payment.amount, 75.0)
        self.assertAlmostEqual(sum(receivable_lines.mapped("debit")), 25.0)
        self.assertAlmostEqual(sum(receivable_lines.mapped("credit")), 100.0)
        self.assertIn(credit_note.name, " | ".join(receivable_lines.mapped("name")))
        self.assertAlmostEqual(invoice.amount_residual, 0.0)
    ...
    def test_payment_register_customer_credit_note_selection_payload_restores_ui_line_state(self):
        invoice = self._create_customer_invoice(100.0)
        credit_note = self._create_customer_refund(25.0)
        ...
        credit_line.is_selected = True
        wizard._onchange_same_side_credit_note_line_ids()
        payload = wizard.same_side_credit_note_selection_payload
        self.assertTrue(payload)

        credit_line.write({"is_selected": False, "amount_to_apply": 0.0})
        wizard.write(
            {
                "amount": 75.0,
                "same_side_credit_note_selection_payload": payload,
            }
        )
    ...
    def test_payment_register_view_merges_credit_notes_into_cross_settlement_panel(self):
        view = self.env.ref(
            "account_partner_settlement.view_account_payment_register_form_inherit_partner_settlement"
        )
        arch = view.arch_db

        self.assertNotIn("Credit Notes in This Payment", arch)
        self.assertNotIn("Payment Journal Items Preview", arch)
        self.assertIn('string="Cross Settlement (AP/AR)"', arch)
        self.assertIn('name="settlement_display_line_ids"', arch)
        self.assertIn('string="Apply/Settle Amount"', arch)
        self.assertIn('name="partner_settlement_amount"', arch)
        self.assertIn('string="Amount"', arch)
        self.assertIn('name="is_selected"', arch)
        self.assertNotIn('widget="boolean_toggle"', arch)

    def test_payment_register_unified_settlement_display_syncs_credit_note_and_cross_lines(self):
        invoice = self._create_customer_invoice(100.0)
        credit_note = self._create_customer_refund(25.0)
        bill = self._create_vendor_bill(40.0)

        wizard = self._create_settlement_wizard(invoice)
        display_credit_line = wizard.settlement_display_line_ids.filtered(
            lambda line: line.line_kind == "credit_note" and line.move_id == credit_note
        )
        display_cross_line = wizard.settlement_display_line_ids.filtered(
            lambda line: line.line_kind == "cross" and line.move_id == bill
        )
        self.assertTrue(display_credit_line)
        self.assertTrue(display_cross_line)

        display_credit_line.is_selected = True
        display_cross_line.is_selected = True
        wizard._onchange_settlement_display_line_ids()

        credit_line = wizard.same_side_credit_note_line_ids.filtered(lambda line: line.move_id == credit_note)
        cross_line = wizard.cross_settlement_line_ids.filtered(lambda line: line.move_id == bill)
        self.assertTrue(credit_line.is_selected)
        self.assertTrue(cross_line.is_selected)
        self.assertAlmostEqual(credit_line.amount_to_apply, 25.0)
        self.assertAlmostEqual(cross_line.amount_to_settle, 40.0)
        self.assertAlmostEqual(wizard.same_side_credit_note_amount, 25.0)
        self.assertAlmostEqual(wizard.cross_settlement_amount, 40.0)
        self.assertAlmostEqual(wizard.amount, 35.0)

    def test_payment_register_vendor_credit_note_selected_in_wizard_is_embedded_in_payment_move(self):
        bill = self._create_vendor_bill(100.0)
        credit_note = self._create_vendor_refund(25.0)

        wizard = self._create_settlement_wizard(bill)
        credit_line = wizard.same_side_credit_note_line_ids.filtered(lambda line: line.move_id == credit_note)
        self.assertTrue(credit_line)
    ...
    def test_payment_register_customer_credit_note_selected_in_wizard_allows_partial_payment(self):
        invoice = self._create_customer_invoice(100.0)
        credit_note = self._create_customer_refund(25.0)
        ...
        self.assertAlmostEqual(wizard.amount, 50.0)
        self.assertAlmostEqual(sum(wizard.allocation_line_ids.mapped("amount_to_pay")), 75.0)
        self.assertAlmostEqual(wizard.payment_difference, 25.0)
        ...
        self.assertTrue(payment.same_side_credit_note_embedded)
        self.assertAlmostEqual(payment.amount, 50.0)
        self.assertAlmostEqual(sum(receivable_lines.mapped("debit")), 25.0)
        self.assertAlmostEqual(sum(receivable_lines.mapped("credit")), 75.0)
        self.assertAlmostEqual(invoice.amount_residual, 25.0)
        self.assertAlmostEqual(credit_note.amount_residual, 0.0)
    ...
    def test_payment_register_cross_settlement_keeps_manual_deduction_on_auto_difference_account(self):
        invoice = self._create_customer_invoice(100.0)
        bill = self._create_vendor_bill(60.0)
        ...
        self.assertAlmostEqual(wizard.payment_difference, 10.0)
        self.assertTrue(wizard.deduction_ids)

        action = wizard.action_create_payments()
        payment = self.env["account.payment"].browse(action["res_id"])
        deduction_lines = payment.move_id.line_ids.filtered(
            lambda line: line.account_id == self.expense_account and line.name == "Difference"
        )
        self.assertAlmostEqual(sum(abs(line.balance) for line in deduction_lines), 10.0)
        self.assertAlmostEqual(invoice.amount_residual, 0.0)
        self.assertAlmostEqual(bill.amount_residual, 0.0)
