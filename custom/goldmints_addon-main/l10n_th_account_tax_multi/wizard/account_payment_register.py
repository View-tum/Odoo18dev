...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_multi_wht_source_move_lines(self):
        """Be context-safe for transient recomputes.

        Transient payment wizard fields can be recomputed during flush/commit where
        `active_model/active_ids` are not present anymore. In that case, skip
        multi-WHT auto logic instead of raising and breaking unrelated payment flows.
        """
        active_ids = self.env.context.get("active_ids", [])
        model = self.env.context.get("active_model")
        if model == "account.move":
            return self.env[model].browse(active_ids).mapped("line_ids")
    ...
    def _compute_payment_difference_handling(self):
        ...
        move_lines = self._get_multi_wht_source_move_lines()
        if not move_lines:
            return res
