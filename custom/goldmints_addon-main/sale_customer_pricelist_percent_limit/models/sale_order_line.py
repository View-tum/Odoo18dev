...
class SaleOrderLine(models.Model):
    ...
    def _should_enforce_pricelist_discount_limit(self):
        user = self.env.user
        return not (user.has_group('sales_team.group_sale_manager') or user.has_group('base.group_system'))
    ...
    def _check_discount_not_exceed_customer_pricelist_percent(self):
        for line in self:
            ...
            if line_disc > max_disc and line._should_enforce_pricelist_discount_limit():
    ...
    def _onchange_discount_limit_from_customer_pricelist(self):
        for line in self:
            ...
            if max_disc is None:
                continue
            if (line.discount or 0.0) > max_disc and line._should_enforce_pricelist_discount_limit():
