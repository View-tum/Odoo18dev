...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        saved_values = {}
        for line in self:
            saved_values[line.id] = {
                'price_unit': line.price_unit,
                'discount': line.discount,
                'fixed_discount': line.fixed_discount,
                'percent_discount': line.percent_discount,
            }

        super()._compute_price_unit_and_date_planned_and_name()

        for line in self:
            saved = saved_values.get(line.id)
            if not saved:
                continue
            if saved['price_unit']:
                line.price_unit = saved['price_unit']
            if saved['percent_discount']:
                line.discount = saved['discount']
                line.percent_discount = saved['percent_discount']
                line.fixed_discount = 0.0
            elif saved['fixed_discount']:
                line.discount = saved['discount']
                line.fixed_discount = saved['fixed_discount']
