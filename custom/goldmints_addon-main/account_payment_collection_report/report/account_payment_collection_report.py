...
class AccountPaymentCollectionReport(models.TransientModel):
    ...
    route_id = fields.Many2one(
        ...
        string="สาย",
        help="(365 custom) สำหรับเลือกสายการส่งเพื่อกรองข้อมูลในรายงาน (จำเป็นเมื่อกรองตามสาย).",
    )
    ...
    customer_ids = fields.Many2many(
        ...
        string="ลูกค้า",
        help="(365 custom) ระบุลูกค้าที่จะรวมในรายงาน (จำเป็นเมื่อกรองตามลูกค้า).",
    )
    ...
    date_from = fields.Datetime(
        ...
        help="(365 custom) วันที่เริ่มต้นและเวลาของช่วงข้อมูล (ค่าเริ่มต้นคือ 08:00 ของวันปัจจุบัน).",
    )
    date_to = fields.Datetime(
        string="วันที่สิ้นสุด",
        help="(365 custom) วันที่สิ้นสุดและเวลาของช่วงข้อมูล (ค่าเริ่มต้นคือ เวลาปัจจุบัน).",
    )
    unprinted_only = fields.Boolean(
        string="เฉพาะใบแจ้งหนี้ที่ยังไม่เคยพิมพ์",
        default=True,
        help="(365 custom) กรองเฉพาะการชำระเงินที่ยังไม่ถูกพิมพ์ในรายงานนี้",
    )
