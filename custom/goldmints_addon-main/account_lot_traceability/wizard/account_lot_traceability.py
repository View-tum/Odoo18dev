...
class AccountLotTraceability(models.TransientModel):
    ...
    lot_id = fields.Many2one(
        ...
        string="หมายเลขล็อต/ซีเรียลนัมเบอร์",
        required=True,
    ...
    allowed_customer_ids = fields.Many2many(
        "res.partner", compute="_compute_allowed_customer_ids"
    )

    customer_id = fields.Many2one(
        comodel_name="res.partner",
        string="ลูกค้า",
        domain="[('id', 'in', allowed_customer_ids)]",
        help="(365 custom) เลือกพนักงานขายหรือลูกค้าที่ต้องการกรอง",
    )
    ...
    def _compute_allowed_customer_ids(self):
        for record in self:
            if not record.lot_id:
                record.allowed_customer_ids = False
                continue

            move_line_domain = [
                ("lot_id.name", "=", record.lot_id.name),
                ("state", "=", "done"),
                ("picking_id.picking_type_id.code", "=", "outgoing"),
            ]

            if record.product_id:
                move_line_domain.append(("product_id", "=", record.product_id.id))

            move_lines = self.env["stock.move.line"].search(move_line_domain)

            partners = (
                move_lines.mapped("move_id.sale_line_id.invoice_lines.move_id")
                .filtered(
            ...
            record.allowed_customer_ids = partners
    ...
    def action_find_invoices(self):
        ...
        for move_line in move_lines:
            ...
            invoice_lines = so_line.invoice_lines.filtered(
                ...
                and (not self.customer_id or x.move_id.partner_id == self.customer_id)
            )
            ...
            for inv_line in invoice_lines:
                ...
                related_pickings = move_lines.filtered(
                    lambda ml: ml.product_id == product
                    and any(
                        il.move_id == invoice
                        for il in ml.move_id.sale_line_id.invoice_lines
                    )
                ).mapped("picking_id")
