...
class AccountPaymentRegister(models.TransientModel):
    ...
    settlement_display_line_ids = fields.One2many(
        "account.payment.register.settlement.display.line",
        "wizard_id",
        string="Settlement Documents",
    )
    ...
    partner_settlement_amount = fields.Monetary(
        string="Amount",
        currency_field="currency_id",
        compute="_compute_partner_settlement_amount",
    )
    same_side_credit_note_selection_payload = fields.Json(copy=False)
    ...
    show_partner_settlement_panel = fields.Boolean(
        compute="_compute_show_partner_settlement_panel",
    )
    ...
    def default_get(self, fields_list):
        ...
        res["settlement_display_line_ids"] = self._prepare_settlement_display_commands_from_command_values(
            res.get("same_side_credit_note_line_ids") or [],
            res.get("cross_settlement_line_ids") or [],
        )
        probe = self.new(res)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            display_commands = sanitized_vals.get("settlement_display_line_ids") or []
            sanitized_display_commands = []
            for command in display_commands:
                if not isinstance(command, (list, tuple)) or len(command) < 3 or command[0] != 0:
                    sanitized_display_commands.append(command)
                    continue
                line_vals = dict(command[2] or {})
                if line_vals.get("move_id") and line_vals.get("line_kind"):
                    sanitized_display_commands.append((command[0], command[1], line_vals))
            if sanitized_display_commands:
                sanitized_vals["settlement_display_line_ids"] = sanitized_display_commands
            else:
                sanitized_vals.pop("settlement_display_line_ids", None)
        ...
        for wizard, vals, shadow_line_values in zip(wizards, sanitized_vals_list, shadow_line_values_list):
            ...
            if not vals.get("settlement_display_line_ids"):
                wizard._refresh_settlement_display_lines()
            if shadow_line_values:
                wizard._apply_cross_settlement_shadow_values(shadow_line_values)
            wizard._sync_settlement_display_to_source_lines()
        return wizards

    def write(self, vals):
        sanitized_vals = dict(vals)
        if "cross_settlement_line_ids" in sanitized_vals:
            sanitized_vals["cross_settlement_line_ids"] = self._sanitize_settlement_source_commands(
                sanitized_vals["cross_settlement_line_ids"],
                required_fields=("move_id",),
            )
        if "same_side_credit_note_line_ids" in sanitized_vals:
            sanitized_vals["same_side_credit_note_line_ids"] = self._sanitize_settlement_source_commands(
                sanitized_vals["same_side_credit_note_line_ids"],
                required_fields=("move_id",),
            )
        if "settlement_display_line_ids" in sanitized_vals:
            sanitized_vals["settlement_display_line_ids"] = self._sanitize_settlement_source_commands(
                sanitized_vals["settlement_display_line_ids"],
                required_fields=("move_id", "line_kind"),
            )
        return super().write(sanitized_vals)

    @api.model
    def _sanitize_settlement_source_commands(self, commands, required_fields):
        sanitized_commands = []
        for command in commands or []:
            ...
            if command_type == 0:
                line_vals = dict(command[2] or {}) if len(command) > 2 else {}
                if all(line_vals.get(field_name) for field_name in required_fields):
                    sanitized_commands.append(Command.create(line_vals))
                continue
            if command_type == 1:
                line_vals = dict(command[2] or {}) if len(command) > 2 else {}
                if line_vals:
                    sanitized_commands.append((command_type, command[1], line_vals))
                continue
            sanitized_commands.append(command)
    ...
    @api.depends(
        "same_side_credit_note_amount",
        "cross_settlement_amount",
        "settlement_display_line_ids.is_selected",
        "settlement_display_line_ids.apply_amount",
    )
    def _compute_partner_settlement_amount(self):
        for wizard in self:
            display_amount = sum(
                wizard.settlement_display_line_ids.filtered("is_selected").mapped("apply_amount")
            )
            wizard.partner_settlement_amount = display_amount or (
                wizard.same_side_credit_note_amount + wizard.cross_settlement_amount
            )
    ...
    @api.depends("show_cross_settlement", "show_same_side_credit_notes")
    def _compute_show_partner_settlement_panel(self):
        for wizard in self:
            wizard.show_partner_settlement_panel = bool(
                wizard.show_cross_settlement or wizard.show_same_side_credit_notes
            )
    ...
    def _compute_amount(self):
        ...
        for wizard in self:
            ...
            default_amount = wizard.currency_id.round(total_amount_values["amount_by_default"])
            if (
                wizard.cross_settlement_line_ids.filtered("is_selected")
                and wizard.currency_id.compare_amounts(wizard.custom_user_amount, default_amount) == 0
            ):
                continue
            preserved_manual_states[wizard._get_preserved_state_key()] = {
                ...
                "deductions": [
                    wizard._get_existing_deduction_values(deduction)
                    for deduction in wizard.deduction_ids
                ],
            }
        ...
        for wizard in self:
            ...
            if preserved_state:
                ...
                if (
                    ...
                    wizard.with_context(skip_wht_deduct=True).payment_difference_handling = preserved_state["handling"]
                if preserved_state.get("deductions") and not wizard.deduction_ids:
                    wizard.with_context(
                    ...
                    ).update(
                        {
                            "deduction_ids": [Command.clear()] + [
                                ...
                                for deduction_values in preserved_state["deductions"]
                            ]
                        }
                    )
                if not wizard._should_preserve_manual_cross_amount() and not wizard._get_manual_non_wht_deduction_lines():
            ...
            if not payment_offset_amount:
                continue
            total_amount_values = wizard._get_total_amounts_to_pay(wizard.batches)
            base_amount = wizard.currency_id.round(total_amount_values["amount_by_default"])
            wizard.amount = wizard.currency_id.round(max(base_amount - payment_offset_amount, 0.0))
    ...
    def _compute_payment_difference(self):
        ...
        for wizard in self:
            wizard._sync_settlement_display_to_source_lines()
        for wizard in self.filtered(lambda record: record.cross_settlement_line_ids.filtered("is_selected")):
            if wizard._should_preserve_manual_cross_amount():
                preserved_amounts[wizard._get_preserved_state_key()] = wizard.amount
            else:
                preserved_amounts[wizard._get_preserved_state_key()] = wizard._get_cross_auto_cash_amount()
        super()._compute_payment_difference()
        for wizard in self:
            preserved_amount = preserved_amounts.get(wizard._get_preserved_state_key())
            if preserved_amount is not None and wizard.currency_id.compare_amounts(wizard.amount, preserved_amount) != 0:
                wizard.amount = preserved_amount
            ...
            if wizard.cross_settlement_line_ids.filtered("is_selected"):
                ...
                if (
                    ...
                    and getattr(wizard, "wht_tax_id", False)
                    and getattr(wizard, "wht_amount_base", 0.0)
                ):
                    wizard.with_context(skip_wht_deduct=True).payment_difference_handling = "reconcile"
                manual_deduction_lines = wizard._get_manual_non_wht_deduction_lines()
                if wizard.payment_difference_handling == "reconcile_multi_deduct" and not manual_deduction_lines:
                    wizard.with_context(skip_wht_deduct=True).payment_difference_handling = "open"
                wizard.payment_difference = max(adjusted_difference, 0.0)
                if not manual_deduction_lines:
                    wizard._cleanup_cross_settlement_auto_deductions(force=True)
                if (
                    wizard._should_sync_allocation_lines()
                    and wizard.currency_id.compare_amounts(wizard.amount, 0.0) > 0
                ):
                    wizard._sync_allocation_lines_to_target_amount(
                        wizard.currency_id.round(wizard.amount + wizard._get_same_side_credit_note_offset_amount())
                    )
                continue
            manual_cross_difference = wizard._get_manual_cross_payment_difference() if wizard.cross_settlement_line_ids.filtered("is_selected") else 0.0
            total_amount_values = wizard._get_total_amounts_to_pay(wizard.batches) if wizard.cross_settlement_line_ids.filtered("is_selected") else {}
            default_amount = wizard.currency_id.round(total_amount_values.get("amount_by_default", 0.0)) if total_amount_values else 0.0
            default_cash_amount = wizard.currency_id.round(
                max(default_amount - wizard._get_current_settlement_wht_amount_from_fields(), 0.0)
            ) if total_amount_values else 0.0
            is_manual_cross_amount = bool(
                wizard.cross_settlement_line_ids.filtered("is_selected")
                and not wizard.currency_id.is_zero(manual_cross_difference)
                and wizard.currency_id.compare_amounts(wizard.amount, default_amount) != 0
                and wizard.currency_id.compare_amounts(wizard.amount, default_cash_amount) != 0
            )
            if is_manual_cross_amount:
                manual_deduction_lines = wizard._get_manual_non_wht_deduction_lines()
                if manual_deduction_lines:
                    wizard.payment_difference = wizard.currency_id.round(sum(manual_deduction_lines.mapped("amount")))
                else:
                    wizard.payment_difference = wizard.currency_id.round(
                        math.floor(wizard._get_manual_cross_payment_difference())
                    )
            else:
                wizard.payment_difference = max(adjusted_difference, 0.0)
            if (
                wizard.cross_settlement_line_ids.filtered("is_selected")
                and wizard._should_sync_allocation_lines()
                and wizard.currency_id.compare_amounts(wizard.amount, 0.0) > 0
            ):
                wizard._sync_allocation_lines_to_target_amount(
                    wizard.currency_id.round(wizard.amount + wizard._get_same_side_credit_note_offset_amount())
                )
            if wizard.payment_difference_handling == "reconcile_multi_deduct" and not wizard._should_preserve_manual_cross_amount():
    ...
    @api.onchange("settlement_display_line_ids")
    def _onchange_settlement_display_line_ids(self):
        for wizard in self:
            wizard._restore_settlement_display_line_metadata()
            wizard._sync_settlement_display_to_source_lines(force=True)
            if wizard.same_side_credit_note_line_ids.filtered("is_selected"):
                wizard._apply_same_side_credit_note_selection()
            wizard._apply_cross_settlement_selection()
            wizard._sync_payment_amount_from_display_selection(force=True)
            wizard._refresh_settlement_display_lines()
    ...
    def _onchange_cleanup_cross_settlement_auto_deductions(self):
        for wizard in self:
            if not wizard._get_manual_non_wht_deduction_lines():
                wizard._cleanup_cross_settlement_auto_deductions()
    ...
    def action_select_all(self):
        ...
        self.same_side_credit_note_line_ids.write({"is_selected": True})
        self._apply_same_side_credit_note_selection()
        self._apply_cross_settlement_selection()
        self._refresh_settlement_display_lines()
        return self._get_payment_register_reload_action()

    def action_unselect_all(self):
        self.ensure_one()
        self.cross_settlement_line_ids.write({"is_selected": False})
        self.same_side_credit_note_line_ids.write({"is_selected": False})
        self._apply_same_side_credit_note_selection()
        self._apply_cross_settlement_selection()
        self._refresh_settlement_display_lines()
        return self._get_payment_register_reload_action()

    def _get_payment_register_reload_action(self):
        self.ensure_one()
        return {
    ...
    def _get_command_values(self, command):
        if not isinstance(command, (list, tuple)) or len(command) < 3 or command[0] != 0:
            return {}
        return dict(command[2] or {})

    def _prepare_settlement_display_command_values(self, line_kind, values):
        move_id = values.get("move_id")
        currency_id = values.get("currency_id")
        account_id = values.get("account_id")
        if not (move_id and currency_id and account_id):
            return {}
        if line_kind == "credit_note":
            open_amount = values.get("residual_amount") or 0.0
            apply_amount = values.get("amount_to_apply") or 0.0
            document_label = "CN"
        else:
            open_amount = values.get("residual_signed_amount") or 0.0
            apply_amount = values.get("amount_to_settle") or 0.0
            move = self.env["account.move"].browse(move_id)
            document_label = self._get_document_label(move) if move else "Document"
        return {
            "line_kind": line_kind,
            "move_id": move_id,
            "currency_id": currency_id,
            ...
            "is_selected": bool(values.get("is_selected")),
            "document_label": document_label,
        }

    def _prepare_settlement_display_commands_from_command_values(self, same_side_commands, cross_commands):
        commands = []
        for command in same_side_commands:
            values = self._get_command_values(command)
            display_values = self._prepare_settlement_display_command_values("credit_note", values)
            if display_values:
                commands.append(Command.create(display_values))
        for command in cross_commands:
            values = self._get_command_values(command)
            display_values = self._prepare_settlement_display_command_values("cross", values)
            if display_values:
                commands.append(Command.create(display_values))
        return commands

    def _prepare_settlement_display_commands_from_records(self):
        self.ensure_one()
        commands = []
                ...
                Command.create(
                    {
                        ...
                        "open_amount": line.residual_amount,
                        "apply_amount": line.amount_to_apply,
                        "is_selected": line.is_selected,
                        "document_label": line.document_label or "CN",
                    }
                )
            )
                ...
                Command.create(
                    {
                        ...
                        "currency_id": line.currency_id.id,
                        "account_id": line.account_id.id,
                        "open_amount": line.residual_signed_amount,
                        "apply_amount": line.amount_to_settle,
                        "is_selected": line.is_selected,
                        "document_label": line.document_label,
                    }
                )
            )
        return commands

    def _refresh_settlement_display_lines(self):
        for wizard in self:
            commands = wizard._prepare_settlement_display_commands_from_records()
            wizard.update({"settlement_display_line_ids": [Command.clear(), *commands] if commands else [Command.clear()]})

    def _restore_settlement_display_line_metadata(self):
        for wizard in self:
            if not wizard.settlement_display_line_ids:
                continue
            if not wizard.same_side_credit_note_line_ids:
                wizard._populate_same_side_credit_note_lines()
            if not wizard.cross_settlement_line_ids:
                wizard._populate_cross_settlement_lines()
            commands = wizard._prepare_settlement_display_commands_from_records()
            template_values = [wizard._get_command_values(command) for command in commands]
            for display_line, values in zip(wizard.settlement_display_line_ids, template_values):
                if display_line.move_id and display_line.line_kind:
                    continue
                origin = display_line._origin if display_line._origin and display_line._origin.exists() else self.env[
                    "account.payment.register.settlement.display.line"
                ]
                if origin:
                    values = {
                        "line_kind": origin.line_kind,
                        "document_label": origin.document_label,
                        "move_id": origin.move_id.id,
                        "currency_id": origin.currency_id.id,
                        "account_id": origin.account_id.id,
                        "open_amount": origin.open_amount,
                        "apply_amount": origin.apply_amount,
                    }
                selected = display_line.is_selected
                apply_amount = display_line.apply_amount
                display_line.update(
                    {
                        "line_kind": values.get("line_kind"),
                        "document_label": values.get("document_label"),
                        "move_id": values.get("move_id"),
                        "currency_id": values.get("currency_id"),
                        "account_id": values.get("account_id"),
                        "open_amount": values.get("open_amount"),
                        "apply_amount": apply_amount or values.get("apply_amount") or 0.0,
                        "is_selected": selected,
                    }
                )

    def _sync_settlement_display_to_source_lines(self, force=False):
        for wizard in self:
            if not wizard.settlement_display_line_ids:
                continue
            if not wizard.same_side_credit_note_line_ids:
            ...
            if not wizard.cross_settlement_line_ids:
                wizard._populate_cross_settlement_lines()
            if wizard.settlement_display_line_ids.filtered(lambda line: not line.move_id or not line.line_kind):
                wizard._restore_settlement_display_line_metadata()
            if not force and not wizard.settlement_display_line_ids.filtered(lambda line: line.is_selected or line.apply_amount):
                continue
            same_side_by_move = {line.move_id.id: line for line in wizard.same_side_credit_note_line_ids}
            cross_by_move = {line.move_id.id: line for line in wizard.cross_settlement_line_ids}
            for display_line in wizard.settlement_display_line_ids:
                amount = display_line.apply_amount or 0.0
                if display_line.is_selected and wizard.currency_id.is_zero(amount):
                    amount = abs(display_line.open_amount or 0.0)
                    display_line.apply_amount = amount
                if display_line.line_kind == "credit_note":
                    source_line = same_side_by_move.get(display_line.move_id.id)
                    if not source_line:
                        continue
                    source_line.is_selected = display_line.is_selected
                    source_line.amount_to_apply = amount if display_line.is_selected else 0.0
                    continue
                source_line = cross_by_move.get(display_line.move_id.id)
                if not source_line:
                    continue
                source_line.is_selected = display_line.is_selected
                source_line.amount_to_settle = amount if display_line.is_selected else 0.0

    def _sync_payment_amount_from_display_selection(self, force=False):
        self.ensure_one()
        if not force and getattr(self, "custom_user_amount", False):
            return
        if not self._should_sync_allocation_lines() or not self.currency_id:
            return
        selected_display_lines = self.settlement_display_line_ids.filtered("is_selected")
        if not selected_display_lines:
            return
        same_side_offset = self._get_same_side_credit_note_offset_amount()
        cross_offset = self._get_cross_payment_offset_amount()
        if self.currency_id.is_zero(same_side_offset + cross_offset):
            return
        _minimum_target, maximum_target = self._get_allocation_target_bounds()
        allocation_target = self._sync_allocation_lines_to_target_amount(
            self.currency_id.round(max(maximum_target - cross_offset, 0.0))
        )
    ...
    def _get_cross_auto_cash_amount(self):
        self.ensure_one()
        if not self.currency_id:
            return 0.0
        total_amount_values = self._get_total_amounts_to_pay(self.batches)
        base_amount = self.currency_id.round(total_amount_values["amount_by_default"])
        return self.currency_id.round(
            max(
                base_amount
                - self._get_payment_offset_amount()
                - self._get_current_settlement_wht_amount_from_fields(),
                0.0,
            )
        )

    def _get_manual_cross_payment_difference(self):
        self.ensure_one()
        if not self.currency_id:
            return 0.0
        total_amount_values = self._get_total_amounts_to_pay(self.batches)
        ...
        difference = (
            base_amount
            - (self.amount or 0.0)
            - self._get_effective_cross_settlement_amount()
            - self._get_manual_cross_source_wht_amount()
            - self._get_same_side_credit_note_offset_amount()
        )
        return self.currency_id.round(max(difference, 0.0))

    def _get_manual_cross_source_wht_amount(self):
        self.ensure_one()
        if "wht_tax_id" not in self._fields or not self.wht_tax_id or not self.wht_amount_base:
            return self._get_current_settlement_wht_amount_from_fields()
        if self.wht_tax_id.is_pit:
            return self._get_current_settlement_wht_amount_from_fields()
        return self.currency_id.round(abs((self.wht_tax_id.amount / 100) * self.wht_amount_base))

    def _should_preserve_manual_cross_amount(self):
        self.ensure_one()
        ...
        if not self.currency_id:
            return False
        total_amount_values = self._get_total_amounts_to_pay(self.batches)
        default_amount = self.currency_id.round(total_amount_values["amount_by_default"])
        default_cash_amount = self.currency_id.round(
            max(default_amount - self._get_current_settlement_wht_amount_from_fields(), 0.0)
        )
        if (
            self.currency_id.compare_amounts(self.amount, default_amount) == 0
            or self.currency_id.compare_amounts(self.amount, default_cash_amount) == 0
        ):
            return False
        if self.currency_id.compare_amounts(self._get_manual_cross_payment_difference(), 0.0) > 0:
            return True
        auto_cash_amount = self._get_cross_auto_cash_amount()
        if self.currency_id.compare_amounts(self.amount, auto_cash_amount) != 0:
            return True
        if not getattr(self, "custom_user_amount", False):
            return False
        return (
            self.currency_id.compare_amounts(self.amount, default_amount) != 0
            and self.currency_id.compare_amounts(self.amount, default_cash_amount) != 0
        )
    ...
    def _get_explicit_cross_payment_offset_amount(self):
        self.ensure_one()
        if not self.currency_id:
            return 0.0
        selected_lines = self.cross_settlement_line_ids.filtered("is_selected")
        explicit_total = 0.0
        explicit_lines = self.env["account.payment.register.cross.settlement.line"]
        for line in selected_lines:
            if not line.amount_to_settle:
                continue
            explicit_lines |= line
            ...
            if line.residual_signed_amount < 0:
                signed_amount *= -1
            explicit_total += signed_amount
        gross_amount = self.currency_id.round(max(explicit_total, 0.0))
        if not gross_amount:
            return 0.0
        wht_amount = self._get_cross_settlement_wht_amount(explicit_lines)
        return self.currency_id.round(max(gross_amount - wht_amount, 0.0))
    ...
    def _is_auto_difference_deduction_line(self, deduction):
        ...
        if not auto_account or deduction.account_id != auto_account:
            return False
        auto_label = getattr(self.company_id, "auto_diff_label", False) or "Difference"
        return (deduction.name or "") == auto_label
    ...
    def _prepare_same_side_credit_note_payment_line_vals(self, batch_lines, payment_type=None):
        self.ensure_one()
        if not self.currency_id:
            return []
        ...
        sign = 1 if (payment_type or self.payment_type) == "inbound" else -1
        vals_list = []
    ...
    def _prepare_selected_same_side_credit_note_payment_line_vals(self, payment_type=None):
        self.ensure_one()
    ...
    def _get_manual_non_wht_deduction_lines(self):
        self.ensure_one()
        if not getattr(self, "deduction_ids", False):
            return self.env["account.payment.deduction"]
        return self.deduction_ids.filtered(
            lambda line: not line.is_open
            and not self._is_wht_deduction_line(line)
            and not self.currency_id.is_zero(line.amount)
        )
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.env.context.get("force_cross_settlement_partial_payment"):
            ...
            manual_deduction_values = [
                deduction_values
                for deduction_values in self.env.context.get("force_cross_settlement_manual_deduction_values", [])
                if not deduction_values.get("is_open")
                and not deduction_values.get("wht_tax_id")
                and not self.currency_id.is_zero(deduction_values.get("amount", 0.0))
            ]
            if manual_deduction_values:
                existing_write_off_line_vals = [
                    line_vals
                ...
                payment_vals["write_off_line_vals"] = existing_write_off_line_vals + [
                    self._prepare_deduct_move_line_from_values(deduction_values)
                    for deduction_values in manual_deduction_values
                ]
                ...
                payment_vals["is_multi_deduction"] = bool(write_off_line_vals)
            else:
    ...
    def _prepare_deduct_move_line_from_values(self, deduction_values):
        conversion_rate = self.env["res.currency"]._get_conversion_rate(
            self.currency_id,
            self.company_id.currency_id,
            self.company_id,
        ...
        amount = deduction_values.get("amount", 0.0)
        ...
        partner = self.env["res.partner"].browse(deduction_values.get("partner_id")).exists() or self.partner_id
        values = {
            "name": deduction_values.get("name"),
            "account_id": deduction_values.get("account_id"),
            "partner_id": partner.id,
            "currency_id": self.currency_id.id,
            "amount_currency": write_off_amount_currency,
            "balance": write_off_balance,
            "analytic_distribution": deduction_values.get("analytic_distribution") or {},
        }
        return values
    ...
    def _cleanup_cross_settlement_auto_deductions(self, force=False):
        self.ensure_one()
        ...
        expected_diff = self._get_cross_expected_deduction_amount()
        keep_total = sum(keep_lines.mapped("amount"))
        if not force and self.currency_id.compare_amounts(expected_diff, keep_total) != 0:
            return
        commands = [Command.clear()]
    ...
    def _apply_cross_settlement_selection(self):
        ...
        if not self.currency_id:
            return
        same_side_offset = self._get_same_side_credit_note_offset_amount()
        effective_cross_offset = self._get_cross_payment_offset_amount()
        explicit_cross_offset = self._get_explicit_cross_payment_offset_amount()
        base_settle_capacity = self._get_base_settle_capacity()
        if self.currency_id.compare_amounts(self.amount + effective_cross_offset, base_settle_capacity) <= 0:
            preserved_cross_offset = effective_cross_offset
        else:
            preserved_cross_offset = explicit_cross_offset
        base_cash_amount = self.currency_id.round(
            self.amount + preserved_cross_offset + same_side_offset
        )
        ...
        target_amount = min(base_settle_capacity, selected_net)
        ...
        if not self.currency_id.is_zero(cash_amount + same_side_offset):
            allocation_target = self._sync_allocation_lines_to_target_amount(
    ...
    def _apply_same_side_credit_note_selection(self):
        ...
        if self.currency_id.compare_amounts(self.amount, cash_amount) != 0:
            self.amount = cash_amount
        self._sync_same_side_credit_note_selection_payload()
        self._compute_payment_difference()

    def _sync_same_side_credit_note_selection_payload(self):
        self.ensure_one()
        if not self.currency_id:
            self.same_side_credit_note_selection_payload = False
            return
        payload = []
        for line in self.same_side_credit_note_line_ids:
            amount = self.currency_id.round(line.amount_to_apply or 0.0)
            if not line.is_selected or not line.move_id or self.currency_id.compare_amounts(amount, 0.0) <= 0:
                continue
            payload.append(
                {
                    "move_id": line.move_id.id,
        ...
        self.same_side_credit_note_selection_payload = payload or False

    def _get_same_side_credit_note_selection_payload_items(self):
        self.ensure_one()
        payload = self.same_side_credit_note_selection_payload or []
    ...
    def _restore_same_side_credit_note_selection_from_payload(self):
        self.ensure_one()
        if self.same_side_credit_note_line_ids.filtered(lambda line: line.is_selected and line.amount_to_apply > 0):
            return
        payload_items = self._get_same_side_credit_note_selection_payload_items()
        if not payload_items:
            return
        if not self.same_side_credit_note_line_ids:
            self._populate_same_side_credit_note_lines()
        lines_by_move = {line.move_id.id: line for line in self.same_side_credit_note_line_ids if line.move_id}
        for item in payload_items:
            move_id = int(item.get("move_id") or 0)
            line = lines_by_move.get(move_id)
            if not line:
                continue
            amount = self.currency_id.round(float(item.get("amount") or 0.0))
            if self.currency_id.compare_amounts(amount, 0.0) <= 0:
                continue
            line.is_selected = True
            line.amount_to_apply = min(amount, line.residual_amount)
    ...
    def _get_created_payment_records(self, action_result):
        ...
        if not isinstance(action_result, dict) or action_result.get("res_model") != "account.payment":
            return self._get_created_payment_records_from_current_lines()
        if action_result.get("res_id"):
            return payment_obj.browse(action_result["res_id"]).exists()
        domain = action_result.get("domain")
        payments = payment_obj.search(domain) if domain else payment_obj
        return payments or self._get_created_payment_records_from_current_lines()

    def _get_created_payment_records_from_current_lines(self):
        payment_obj = self.env["account.payment"]
        current_lines = self.line_ids.filtered(
            lambda line: line.account_id.account_type in payment_obj._get_valid_payment_account_types()
        )
        partials = current_lines.matched_debit_ids | current_lines.matched_credit_ids
        payment_moves = (partials.debit_move_id | partials.credit_move_id).move_id
        return payment_obj.search([("move_id", "in", payment_moves.ids)]) if payment_moves else payment_obj
    ...
    def _reconcile_selected_same_side_credit_notes(self, payments):
        ...
        payment_lines = payment_moves.line_ids.filtered(
            lambda line: line.parent_state == "posted"
            and line.account_type in self.env["account.payment"]._get_valid_payment_account_types()
        )
        for selected_line in selected_lines:
            ...
            for payment_line in candidate_payment_lines:
                self._unlink_internal_payment_line_reconciliations(payment_line, payment_moves)
                open_lines = self._get_open_move_lines(
                    selected_line.move_id,
                    selected_line.account_id.account_type,
                )
        ...
        self._reconcile_current_lines_after_same_side_credit_notes(payments)

    def _unlink_internal_payment_line_reconciliations(self, payment_line, payment_moves):
        internal_partials = (payment_line.matched_debit_ids | payment_line.matched_credit_ids).filtered(
            lambda partial: partial.debit_move_id.move_id in payment_moves
            and partial.credit_move_id.move_id in payment_moves
        )
        if internal_partials:
            involved_lines = internal_partials.debit_move_id | internal_partials.credit_move_id
            internal_partials.unlink()
            involved_lines.invalidate_recordset(["amount_residual", "amount_residual_currency", "reconciled"])

    def _reconcile_current_lines_after_same_side_credit_notes(self, payments):
        self.ensure_one()
        current_account_type = self._get_current_account_type()
        current_lines = self.line_ids.filtered(
            lambda line: line.account_id.account_type == current_account_type and not line.reconciled
        )
        if not current_lines:
            return
        payment_lines = payments.move_id.line_ids.filtered(
            lambda line: line.parent_state == "posted"
            and line.account_id.account_type == current_account_type
            and line.partner_id.commercial_partner_id == self.partner_id.commercial_partner_id
            and not line.reconciled
        )
        for current_line in current_lines.sorted(lambda line: (line.date_maturity or line.date, line.id)):
            candidate_payment_lines = payment_lines.filtered(
                lambda line: not line.reconciled
                and line.account_id == current_line.account_id
                and line.balance * current_line.balance < 0
            )
            for payment_line in candidate_payment_lines:
                if current_line.reconciled:
                    break
                (current_line | payment_line).reconcile()
    ...
    def _validate_same_side_credit_note_inputs(self):
        self.ensure_one()
        self._restore_same_side_credit_note_selection_from_payload()
    ...
    def _check_deduction_amount(self):
        if self.env.context.get("skip_wht_deduct") or self.env.context.get("in_wht_sync"):
            return
        for wizard in self:
            if not wizard.cross_settlement_line_ids.filtered("is_selected"):
                continue
            if wizard.payment_difference_handling != "reconcile_multi_deduct":
                continue
            manual_deductions = wizard._get_effective_deduction_lines().filtered(
                lambda deduction: not wizard._is_wht_deduction_line(deduction)
            )
    ...
    def action_create_payments(self):
        self.ensure_one()
        preserved_manual_amount = self.amount if getattr(self, "custom_user_amount", False) else None
        preserved_manual_currency = self.custom_user_currency_id if getattr(self, "custom_user_currency_id", False) else False
        preserved_handling = self.payment_difference_handling
        preserved_manual_deduction_values = [
            self._get_existing_deduction_values(deduction)
            for deduction in self.deduction_ids.filtered(
                lambda line: not line.is_open
                and not self._is_wht_deduction_line(line)
                and not self.currency_id.is_zero(line.amount)
            )
        ]
        self._restore_settlement_display_line_metadata()
        self._sync_settlement_display_to_source_lines()
        self._apply_same_side_credit_note_selection()
        self._apply_cross_settlement_selection()
        self._sync_payment_amount_from_display_selection()
        if preserved_manual_amount is not None:
            target_amount = self._sync_allocation_lines_to_target_amount(preserved_manual_amount)
            self.amount = target_amount
            self.custom_user_amount = preserved_manual_amount
            self.custom_user_currency_id = preserved_manual_currency or self.currency_id
            self.with_context(skip_wht_deduct=True).payment_difference_handling = preserved_handling
        if not self._should_preserve_manual_cross_amount() and not self._get_manual_non_wht_deduction_lines():
        ...
        if self.currency_id.compare_amounts(self.amount, 0.0) > 0:
            ...
            payment_context = {
                **current_settlement_wht_context,
                ...
                "force_cross_settlement_manual_deduction_values": [
                    deduction_values
                    for deduction_values in (preserved_payment_state.get("deductions", []) or preserved_manual_deduction_values)
                    if not deduction_values.get("is_open")
                    and not deduction_values.get("wht_tax_id")
                    and not self.currency_id.is_zero(deduction_values.get("amount", 0.0))
                ],
            }
            if not payment_wizard._get_manual_non_wht_deduction_lines():
...
class AccountPaymentRegisterSettlementDisplayLine(models.TransientModel):
    _name = "account.payment.register.settlement.display.line"
    _description = "Account Payment Register Settlement Display Line"
    _order = "line_kind desc, move_date, id"

    wizard_id = fields.Many2one("account.payment.register", required=True, ondelete="cascade")
    line_kind = fields.Selection(
        [
            ("credit_note", "Credit Note"),
            ("cross", "Cross Settlement"),
        ],
        required=True,
    )
    is_selected = fields.Boolean(string="Select")
    document_label = fields.Char(string="Type")
    ...
    def _set_selected(self, selected):
        ...
        for line in self:
            ...
            if selected and line.wizard_id.currency_id.is_zero(line.apply_amount):
                line.apply_amount = abs(line.open_amount or 0.0)
            if not selected:
                line.apply_amount = 0.0
