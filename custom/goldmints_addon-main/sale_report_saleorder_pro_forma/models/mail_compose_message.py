...
class MailComposeMessage(models.TransientModel):
    ...
    def _run_jasper_direct(self, orders, fmt="pdf", lang=None):
        ...
        jasper_report = self.env.ref(
            "sale_report_saleorder_pro_forma.jasper_proforma_invoice_en"
        )
        if not jasper_report.exists():
            raise UserError(_("Jasper report not found."))
    ...
    def _run_jasper_direct_th(self, orders, fmt="pdf", lang=None):
        ...
        jasper_report = self.env.ref(
            "sale_report_saleorder_pro_forma.jasper_proforma_invoice_th"
        )
        if not jasper_report.exists():
            raise UserError(_("Jasper report not found."))
