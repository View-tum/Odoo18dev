...
class AccountAgedReceiveableExtension(models.TransientModel):
    ...
    payment_term_extension = fields.Selection(
        selection_add=[
            ("Payment Term Sales", "เงื่อนไขการชำระเงินจาก Sales"),
            ("Payment Term Customer", "เงื่อนไขการชำระเงินจากลูกค้า"),
        ],
        ondelete={
    ...
    region_type = fields.Selection(
        ...
        string="สูตรช่วงระยะเวลาเครดิตตามพื้นที่",
        default="bkk",
        help="(365 custom) เลือกประเภทพื้นที่สำหรับสูตรใน Excel: กรุงเทพและปริมณฑล หรือ ต่างจังหวัด",
    )
    ...
    def _get_data(self):
        ...
        if self.subregion_ids and not self.customer_ids:
        ...
        sql_query = f"""
            SELECT 
                ...
                CASE WHEN (%(date_at)s - {adjusted_date_expression}) <= 0 THEN {amount_field_sql} ELSE 0 END as amount_not_due,
                CASE WHEN (%(date_at)s - {adjusted_date_expression}) BETWEEN 1 AND 30 THEN {amount_field_sql} ELSE 0 END as amount_1_30,
                CASE WHEN (%(date_at)s - {adjusted_date_expression}) BETWEEN 31 AND 60 THEN {amount_field_sql} ELSE 0 END as amount_31_60,
                CASE WHEN (%(date_at)s - {adjusted_date_expression}) BETWEEN 61 AND 90 THEN {amount_field_sql} ELSE 0 END as amount_61_90,
                CASE WHEN (%(date_at)s - {adjusted_date_expression}) > 90 THEN {amount_field_sql} ELSE 0 END as amount_over_90
            ...
            LEFT JOIN (
                SELECT partner_id, MIN(next_run) as min_next_run
                FROM pps_schedule WHERE schedule_type = 'payment' GROUP BY partner_id
    ...
    def action_preview(self):
        ...
        if not results:
            return self._show_notification("ไม่พบข้อมูล", "ไม่มีใบแจ้งหนี้ที่ตรงกับเงื่อนไขที่เลือก")
            ...
            Command.create(
                {
                    ...
                    "days_overdue": max(res["days_overdue"], 0),
