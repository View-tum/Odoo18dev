...
from odoo.addons.effective_date_change.wizard import change_effective_wizard
from odoo.exceptions import ValidationError


# Neutralize the original wizard logic safely
def silent_update_effective_date(self):
    return super(change_effective_wizard.ChangeEffectiveWizard, self).update_effective_date()

change_effective_wizard.ChangeEffectiveWizard.update_effective_date = silent_update_effective_date
...
class ChangeEffectiveWizard(models.TransientModel):
    ...
    def update_effective_date(self):
        # By-passing vendor logic via monkey-patch
        super(ChangeEffectiveWizard, self).update_effective_date()
                    ...
                    if picking.picking_type_id.code == 'incoming' and int(purchase_order.currency_id.id) != system_default_currency:
                        ...
                        for line in picking.move_ids_without_package:
                            ...
                            if product_id in po_details:
                                ...
                                if line_qty <= po_product['quantity']:
                                    total_value = po_product['price_unit'] * line_qty * rate
                                    price_unit[product_id] = total_value
                                    po_details[product_id]['quantity'] -= line_qty
                                else:
                                    total_value = po_product['price_subtotal'] * rate
                                    price_unit[product_id] = total_value
                                    line_qty -= po_product['quantity']
