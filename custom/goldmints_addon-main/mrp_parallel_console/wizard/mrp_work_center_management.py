...
class MrpWorkCenterAdjustQtyWizard(models.TransientModel):
    ...
    def _recompute_distribution(self):
        ...
        for wiz in self:
            ...
            lines = wiz.work_center_line_ids.filtered(
                lambda line: line.workorder_id.state != "cancel"
            )
            if not lines:
                continue
            ...
            locked = lines.filtered(
                lambda line: line.locked
                or (
                    line.new_qty is not None
                    and line.original_qty is not None
                    and int(round(line.new_qty)) != int(round(line.original_qty))
                )
            )
            free = lines - locked
    ...
    def action_adjust_quantities(self):
        ...
        for line in self.work_center_line_ids.filtered(lambda rec: rec.workorder_id.state != "cancel"):
