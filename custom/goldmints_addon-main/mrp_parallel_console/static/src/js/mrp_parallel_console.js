...
export class ParallelShopfloorHome extends Component {
    ...
    async confirmMo(mo) {
        this.dialogService.add(ConfirmationDialog, {
            body: _t("Confirm manufacturing order %s?", mo.name),
            confirm: async () => {
    ...
    async closeMo(mo) {
        this.dialogService.add(ConfirmationDialog, {
            body: _t("Close manufacturing order %s? This may trigger backorder dialogs.", mo.name),
            confirm: async () => {
...
export class QtyEditDialog extends Component {
    ...
    save() {
        ...
        if (!qty || qty <= 0) {
            this.notification.add(_t("Quantity must be greater than zero."), { type: "warning" });
            return;
        }
...
export class MoldWarningDialog extends Component {
    continueAnyway() {
...
MoldWarningDialog.template = "mrp_parallel_console.MoldWarningDialog";
MoldWarningDialog.components = { Dialog };
MoldWarningDialog.props = {
    workorder: Object,
    warning: Object,
    onContinue: Function,
    onChangeMold: Function,
    onCancel: Function,
    close: Function,
};
...
export class ParallelWorkorderConsole extends Component {
    ...
    async loadData(productionId = null, options = {}) {
        ...
        if (Object.prototype.hasOwnProperty.call(res, "can_start_production")) {
            this.state.canStartProduction = !!res.can_start_production;
        } else {
            this.state.canStartProduction = true;
        }
        this.state.moClosed = !!res.mo_closed;
        this.state.productionState = res.production_state || null;
        const fallbackTracking = firstWo && firstWo.production_tracking ? firstWo.production_tracking : null;
        this.state.productionTracking = res.production_tracking || fallbackTracking || null;
        this.state.productionLotId = res.production_lot_id || null;
        this.state.productionLotName = res.production_lot_name || "";
        this.state.mpc_supervisor_checked = res.mpc_supervisor_checked || false;
        this.state.mpc_supervisor_name = res.mpc_supervisor_name || "";
        this.state.mpc_supervisor_check_date = res.mpc_supervisor_check_date || "";
    ...
    async openMoldSelection(wo, preloadedAction = null) {
        const action = preloadedAction || await this._fetchMoldSelectionAction(wo);
        if (!action) {
            return;
        }
    ...
    async _fetchMoldSelectionAction(wo) {
        const res = await rpc("/mrp_parallel_console/get_mold_selection_action", {
            workorder_id: wo.id,
        });
        if (res && res.error) {
            this.notification.add(res.error, { type: "danger" });
    ...
    async _promptMoldWarning(wo, warning) {
        return new Promise((resolve) => {
            this.dialogService.add(MoldWarningDialog, {
                workorder: wo,
                warning,
                onContinue: () => resolve("continue"),
                onChangeMold: () => resolve("change"),
    ...
    async removeWorkorder(wo) {
        this.dialogService.add(ConfirmationDialog, {
            body: _t("Remove Station '%s' from %s?", wo.workcenter_name, wo.name),
            confirm: async () => {
    ...
    async onAddQtyChange(wo, ev) {
        const qtyToAdd = parseFloat(ev.target.value) || 0;
        const oldQty = wo.console_qty || 0;
        ...
        try {
            const userName = session.partner_display_name || session.name || "Unknown User";
            const note = `${userName}`;
            ...
            const total = res.total ?? (oldQty + qtyToAdd);
            wo.console_qty = total;
            wo.qty_logs = res.logs || [];
            this._syncCardQty(wo.id, total, res.logs || []);

            // Reset the input field on the card to empty
            ev.target.value = "";

            this.notification.add(_t("Quantity added: +%s (Total: %s)", qtyToAdd, total), { type: "success" });
        } catch (error) {
            this.notification.add(
                error?.message || "Failed to add quantity.",
                { type: "danger" }
            );
        }
    }
    ...
    openEmployeesPicker() {
        if (this.state.moClosed) {
            this.notification.add(
                _t("This manufacturing order is already closed. Employees cannot be updated."),
                { type: "warning" }
            );
            return;
        }
        ...
        if (!validWos.length) {
            this.notification.add(
                _t("Selected stations are locked (Busy/Maintenance)."),
                { type: "danger" }
            );
            return;
        }
        ...
        this.dialogService.add(SelectCreateDialog, {
            ...
            onSelected: async (employeeIds) => {
                ...
                if (!ids.length) {
                    this.dialogService.add(ConfirmationDialog, {
                        body: _t("Remove all employees from the selected tasks?"),
                        confirm: async () => {
    ...
    async openDeleteWorkCentersWizard() {
        if (!this.productionId) {
            this.notification.add(_t("No production ID available"), { type: "warning" });
            return;
        }
    ...
    openPickingDrawer(wo) {
        return this.openNativeComponents(wo);
    }

    async openNativeComponents(wo) {
        if (!this.productionId) {
        ...
        await this.actionService.doAction("mrp_parallel_console.action_mrp_production_minimal", {
            ...
            props: {
                resId: this.productionId,
            },
            onClose: () => {
                this.state.lotsReviewed = true;
                this.loadData(null, { preserveSelection: true });
            }
        });
    }
    ...
    onEmployeeQtyChange(employeeId, value) {
        if (!this.state.activeWo) return;
        if (!this.state.activeWo.mpc_employee_production_ids) {
            this.state.activeWo.mpc_employee_production_ids = [];
        }
        const qty = parseFloat(value) || 0;
        const existing = this.state.activeWo.mpc_employee_production_ids.find(p => p.employee_id === employeeId);
        if (existing) {
            existing.qty = qty;
        } else {
            this.state.activeWo.mpc_employee_production_ids.push({
    ...
    getEmployeeProductionQty(employeeId) {
        if (!this.state.activeWo || !this.state.activeWo.mpc_employee_production_ids) return 0;
        const entry = this.state.activeWo.mpc_employee_production_ids.find(p => p.employee_id === employeeId);
        return entry ? entry.qty : 0;
    }

    sumEmployeeProds() {
        if (!this.state.activeWo || !this.state.activeWo.mpc_employee_production_ids) return 0;
        return this.state.activeWo.mpc_employee_production_ids.reduce((acc, p) => acc + p.qty, 0);
    }

    getEmployeesForProductionTab() {
        if (!this.state.activeWo) return [];
        const wo = this.state.activeWo;
        const assignedEmps = wo.employees || [];
        const savedProds = wo.mpc_employee_production_ids || [];

        const empMap = new Map();
        for (const emp of assignedEmps) {
            empMap.set(emp.id, { id: emp.id, name: emp.name, fromAssigned: true });
        }
        for (const prod of savedProds) {
            if (!empMap.has(prod.employee_id)) {
                empMap.set(prod.employee_id, {
                    id: prod.employee_id,
                    name: prod.employee_name || `Employee #${prod.employee_id}`,
                    fromSaved: true
                });
            }
        }
    ...
    async saveEmployeeProductions() {
        if (!this.state.activeWo) return;
        const wo = this.state.activeWo;
        try {
            const res = await rpc("/mrp_parallel_console/save_employee_productions", {
                workorder_id: wo.id,
                productions: wo.mpc_employee_production_ids || [],
                note: wo.mpc_employee_production_note || ""
            });

            if (res && res.error) {
                this.notification.add(res.error, { type: "danger" });
            } else {
                this.notification.add(_t("Employee breakdown updated."), { type: "success" });
                // Sync back to the main list
                const mainWo = this.state.workorders.find(w => w.id === wo.id);
                if (mainWo) {
                    mainWo.mpc_employee_production_ids = JSON.parse(JSON.stringify(wo.mpc_employee_production_ids));
                    mainWo.mpc_employee_production_note = wo.mpc_employee_production_note;
                }
            }
        } catch (error) {
            this.notification.add(error.message || "Failed to save breakdown.", { type: "danger" });
        }
    }
    ...
    async _syncDrawerEmployees() {
        if (!this.state.activeWo) {
            return;
        }
        const woId = this.state.activeWo.id;
        const employeeIds = (this.state.activeWo.employees || []).map(e => e.id);

        try {
            await rpc("/mrp_parallel_console/assign_employees", {
                workorder_ids: [woId],
                employee_ids: employeeIds
            });
            // No notification to keep it quiet and "standard-feel"
        } catch (error) {
    ...
    async addQtyLog() {
        ...
        if (qty <= 0) {
            this.notification.add(_t("Please enter a quantity greater than zero."), { type: "warning" });
            return;
        }
        try {
            const res = await rpc("/mrp_parallel_console/add_qty_log", {
            ...
            this.notification.add(
                error?.message || _t("Failed to add quantity log."),
                { type: "danger" }
            );
        }
    }
        ...
        this.dialogService.add(QtyEditDialog, {
            ...
            onSave: async (qty, note) => {
                try {
                    ...
                    this.notification.add(
                        error?.message || _t("Failed to update quantity log."),
                        { type: "danger" }
                    );
                }
            }
    ...
    async openTimeTrackingPopup(log = null) {
        ...
        try {
            ...
            this.notification.add(
                error?.message || _t("Unable to open time tracking."),
                { type: "danger" }
            );
        }
    }
    ...
    async saveActiveWo() {
        ...
        try {
            // 1. Save main console data
            await rpc("/mrp_parallel_console/update_console", { lines: [line] });

            // 2. Save Employee Production data (always save to preserve entries)
            if (wo.mpc_employee_production_ids) {
                const epRes = await rpc("/mrp_parallel_console/save_employee_productions", {
                    workorder_id: wo.id,
                    productions: wo.mpc_employee_production_ids,
                    note: wo.mpc_employee_production_note || ""
                });
                if (epRes && epRes.error) {
                    this.notification.add(epRes.error, { type: "danger" });
                }
            }

            this.notification.add(_t("Work order updated."), { type: "success" });
            this.state.activeWo = null;
            await this.loadData();
        } catch (error) {
            this.notification.add(error.message || _t("Failed to update workorder."), { type: "danger" });
        }
    }

    async toggleSupervisorCheck() {
        if (!this.state.activeWo) return;
        const wo = this.state.activeWo;
        const checked = !wo.mpc_supervisor_checked;
        try {
            const res = await rpc("/mrp_parallel_console/toggle_supervisor_check", {
                workorder_id: wo.id,
                checked: checked
            });
            if (res && res.error) {
            ...
            wo.mpc_supervisor_checked = res.mpc_supervisor_checked;
            wo.mpc_supervisor_name = res.mpc_supervisor_name;
            wo.mpc_supervisor_check_date = res.mpc_supervisor_check_date;
            this.notification.add(_t(checked ? "Supervisor checked." : "Supervisor check removed."), { type: "success" });
        } catch (error) {
            this.notification.add(error.message || _t("Failed to update supervisor check."), { type: "danger" });
        }
    }

    async toggleSupervisorCheckFromCard(wo) {
        if (!wo) return;
        const checked = !wo.mpc_supervisor_checked;
        try {
            const res = await rpc("/mrp_parallel_console/toggle_supervisor_check", {
    ...
    async toggleMoSupervisorCheck() {
        if (!this.productionId) return;
        const checked = !this.state.mpc_supervisor_checked;
        try {
            const res = await rpc("/mrp_parallel_console/toggle_mo_supervisor_check", {
                production_id: this.productionId,
                checked: checked
            });
            if (res && res.error) {
            ...
            this.state.mpc_supervisor_checked = res.mpc_supervisor_checked;
            this.state.mpc_supervisor_name = res.mpc_supervisor_name;
            this.state.mpc_supervisor_check_date = res.mpc_supervisor_check_date;
            this.notification.add(_t(checked ? "MO Supervisor checked." : "MO Supervisor check removed."), { type: "success" });
        } catch (error) {
            this.notification.add(error.message || _t("Failed to update MO supervisor check."), { type: "danger" });
        }
    }
    ...
    async applyConsole() {
        ...
        if (!ids.length) {
            this.dialogService.add(ConfirmationDialog, {
                body: _t("Close production for all workorders in this manufacturing order?"),
                confirm: async () => {
    ...
    async _executeApplyConsole(ids) {
        if (!ids.length) {
            this.notification.add(_t("No work orders to close."), { type: "warning" });
            return;
        }
        ...
        if (res && res.status === "error" && res.action) {
        ...
        if (res && res.status === "validation_failed") {
            this.notification.add(
                res.error || "Validation failed. Please check all production quantities.",
        ...
        if (res && res.status === "missing_component_lots") {
            this.notification.add(
                res.error || _t("Insufficient lots for components. Please review."),
                { type: "danger", sticky: true }
            );
            // Solution: Automatically open the Component Lots screen
            await this.openNativeComponents();
            return;
        }

        if (res && res.status === "overproduction_transfer" && res.action) {
            const action = res.action;
            this.dialogService.add(ConfirmationDialog, {
                title: _t("MO ผลิตเกินเป้าหมาย"),
                body: res.error || _t("MO ผลิตเกินเป้าหมาย ทำให้ต้องเบิกวัตถุดิบเพิ่มเติม กด OK เพื่อไปยังหน้าเบิกวัตถุดิบ"),
                confirm: async () => {
        ...
        if (res && res.scrap_skipped_count) {
            this.notification.add(
                _t("%s scrap record(s) were not validated (insufficient stock / missing lot).", res.scrap_skipped_count),
                { type: "warning" }
            );
        }
        ...
        if (validAction) {
            ...
            this.notification.add(_t("Production close request sent."), {
                type: "success",
            });
            this.state.selectedIds = new Set();
            await this.loadData(null, { preserveSelection: true });
    ...
    async openPickingDrawer(wo = null) {
        const targetWo = wo || this.state.activeWo || (this.state.workorders || [])[0];
        if (!targetWo) {
            this.notification.add(
                _t("No workorder available for component lots."),
                { type: "warning" }
            );
            return;
        }
        return this.openNativeComponents(targetWo);
    }
    ...
    async _openNextPickingAction(pickingId = null) {
        try {
            ...
            this.notification.add(
                error.message || _t("Failed to open picking."),
                { type: "danger" }
            );
        }
    }
    ...
    async _handlePickingActionClose(nextPickingId) {
        if (nextPickingId) {
            this.dialogService.add(ConfirmationDialog, {
                body: _t("Another component picking is pending. Do you want to open it now?"),
                confirm: async () => {
                    await this._openNextPickingAction(nextPickingId);
                },
                cancel: () => {
                    this.notification.add(_t("Stopped automatic picking chain."), {
                        type: "info",
                    });
                },
            });
    ...
    async printCurrentPicking(reportXmlId = null) {
        if (!this.productionId) {
            this.notification.add(
                _t("Please open a manufacturing order before printing."),
                { type: "warning" }
            );
            return;
        }
        if (!this.state.pickingDrawer.activePickingId) {
            this.notification.add(
                _t("No component picking available to print."),
                { type: "warning" }
            );
            return;
        }
        try {
            ...
            if (res.action) {
                ...
                this.notification.add(_t("No print action found."), {
                    type: "warning",
                });
            }
        } catch (error) {
            this.notification.add(
                error.message || _t("Failed to print picking."),
                { type: "danger" }
            );
        }
    }
    ...
    async printProductionAction(actionId, actionModel) {
        if (!this.productionId) {
            this.notification.add(_t("Please open a manufacturing order before printing."), {
                type: "warning",
            });
            return;
        }
        try {
            ...
            if (res.action) {
                ...
                this.notification.add(_t("Print action executed."), { type: "success" });
            }
        } catch (error) {
            this.notification.add(
                error.message || _t("Failed to print manufacturing order."),
                { type: "danger" }
            );
        }
    }
    ...
    async validatePicking() {
        ...
        try {
            ...
            const finalizeSuccess = async () => {
                ...
                await this.loadData();
                if (this.state.detailTab === 'components') {
                    await this._ensureComponentPickingLoaded(true);
                }
            };

            const shouldClose = drawer.open;
            if (shouldClose) {
                drawer.open = false;
                drawer.picking = null;
    ...
    onScrapLotChange(ev) {
        const value = parseInt(ev.target.value, 10);
        const lotId = Number.isNaN(value) ? null : value;
        this.scrapChange("lotId", lotId);

        const lots = this.state.scrapModal.availableLots || [];
        const lotMeta = lots.find(l => l.id === lotId);
        if (lotMeta) {
            this.state.scrapModal.lotName = lotMeta.name;
            // Use lot location if available, otherwise fall back to product's source location
            const productMeta = (this.state.scrapModal.products || []).find(p => p.id === this.state.scrapModal.productId);
            this.state.scrapModal.locationId = lotMeta.location_id || (productMeta && productMeta.source_location_id) || this.state.scrapModal.defaultLocationId;
        } else {
            this.state.scrapModal.lotName = "";
        }
    }
    ...
    _applyScrapProductDefaults(productMeta) {
        ...
        // Baseline Source Location (from product meta or global default)
        let targetLocationId = (productMeta && productMeta.source_location_id) || this.state.scrapModal.defaultLocationId;

        // Auto-select Lot and try to refine location
        if (productMeta && productMeta.type === "finished") {
            ...
            this.state.scrapModal.lotId = firstLot.id;
            this.state.scrapModal.lotName = firstLot.name;
            // Refine location if the lot has one
            if (firstLot.location_id) {
                targetLocationId = firstLot.location_id;
            }
        } else {
            this.state.scrapModal.lotId = null;
            this.state.scrapModal.lotName = "";
        ...
        this.state.scrapModal.locationId = targetLocationId;
    }
    ...
    async startWo(wo) {
        ...
        if (!this.state.canStartProduction) {
            this.notification.add("This manufacturing order is not open for production.", { type: "warning" });
            return;
        }
        ...
        for (const id of targetIds) {
            ...
            if (card.state !== "ready" && card.state !== "waiting" && card.state !== "pending") {
                skippedNotReady.push(id);
                continue;
            }
            if (card.mpc_require_employee_production && (!card.employees || !card.employees.length)) {
        ...
        if (!readyIds.length) {
            ...
            if (skippedNoEmployees.length) {
                ...
                msg = _t("No selected workorders are in a startable state (Ready/Waiting/Pending).");
            }
        ...
        for (const id of readyIds) {
            ...
            if (res && res.status === "mold_warning" && card) {
                const choice = await this._promptMoldWarning(card, res);
                if (choice === "change") {
                    await this.openMoldSelection(card, res.action || null);
                    skippedMoldWarning += 1;
                    continue;
                }
                if (choice !== "continue") {
                    skippedMoldWarning += 1;
                ...
                res = await rpc("/mrp_parallel_console/start_workorder", {
                    workorder_id: id,
                    allow_full_mold: true,
                });
            }
            if (res && res.error) {
                errorCount += 1;
    ...
    openAssignLot() {
        if (!this._productionId) {
            this.notification.add("No manufacturing order selected.", { type: "warning" });
            return;
        }
        const ctx = {
            default_production_id: this._productionId,
            active_id: this._productionId,
            active_model: "mrp.production",
        };
        if (this.state.productionLotId) {
            ctx.default_lot_id = this.state.productionLotId;
        }
        this.actionService.doAction(
            {
                type: "ir.actions.act_window",
            ...
            {
                onClose: () => {
                    this.loadData(this._productionId, { preserveSelection: true });
                },
            }
        );
    }
    ...
    async finishWo(wo) {
        if (this.state.moClosed) {
            this.notification.add(
                _t("This manufacturing order is closed. Finishing workorders is not allowed."),
                { type: "warning" }
            );
            return;
        }

        // --- Employee Production Validation ---
        if (wo.mpc_require_employee_production) {
            const totalEmpQty = (wo.mpc_employee_production_ids || []).reduce((acc, p) => acc + p.qty, 0);
            const doneQty = wo.console_qty || 0;
            if (Math.abs(totalEmpQty - doneQty) > 0.0001) {
                this.notification.add(
                    _t("Total breakdown quantity (%1) must match the 'Done' quantity (%2). Please update the 'Employee Production' tab in Details.").replace('%1', totalEmpQty).replace('%2', doneQty),
                    { type: "danger" }
                );
                this.openDrawer(wo);
                this.setDetailTab('production');
                return;
            }
        }
        ...
        this.dialogService.add(ConfirmationDialog, {
            ...
            body: _t("Mark workorder %1 as completed?").replace('%1', wo.name),
            confirm: async () => {
                try {
                    ...
                    this.notification.add(_t("Workorder marked as done successfully."), { type: "success" });
                    ...
                    this.notification.add(
                        error.message || _t("Failed to finish workorder."),
                        { type: "danger" }
                    );
                }
            },
    ...
    async quickDone(wo) {
        ...
        if (this.state.moClosed) {
            this.notification.add(
                "This manufacturing order is closed.",
                { type: "warning" }
            );
            return;
        }
        ...
        if (wo.mpc_require_employee_production && (!wo.employees || !wo.employees.length)) {
            this.notification.add(_t("Please assign employees before starting."), { type: "warning" });
            return;
        }

        const plannedQty = wo.planned_qty || 0;
        const doneQty = wo.console_qty || 0;
        const remaining = plannedQty - doneQty;

        if (remaining <= 0) {
            this.notification.add(_t("No remaining quantity to produce."), { type: "warning" });
            return;
        }

        try {
            const res = await rpc("/mrp_parallel_console/quick_done", {
                workorder_id: wo.id,
                qty: remaining,
            });
            ...
            if (card) {
                const nextState = res.state || "done";
                const updates = {
                    console_qty: plannedQty,
                    state: nextState,
                    state_label: this._getWorkorderStateLabel(nextState),
                    console_date_start: res.start || card.console_date_start || new Date().toISOString(),
                    console_date_finished: res.end || new Date().toISOString(),
                };
                Object.assign(card, updates);
                this._syncActiveWo(card.id, updates);
            ...
            this.notification.add(_t("Quick Done: Started, produced %1, and marked as done.").replace('%1', remaining), { type: "success" });

            await this.reloadWorkorders();

        } catch (error) {
            console.error("Error in quick done:", error);
            this.notification.add(
                error.message || _t("Failed to complete quick done."),
                { type: "danger" }
            );
        }
    }
