...
class MrpProduction(models.Model):
    ...
    def _console_update_component_qty_done_per_wo(self):
        ...
        for mo in self:
            ...
            wo_round = mo.product_uom_id.rounding or 0.000001

            for move in mo.move_raw_ids.filtered(lambda m: m.state != "cancel"):
                total_required = move.product_uom_qty or 0.0
                if float_is_zero(total_required, precision_rounding=move.product_uom.rounding or 0.000001):
                    continue
                if not mo.product_qty:
                    continue
                factor = total_required / mo.product_qty
                move_round = move.product_uom.rounding or 0.000001
                ...
                for wo in workorders:
                    ...
                    required_qty = float_round(factor * produced_qty, precision_rounding=move_round)
                    ...
                    for idx, line in enumerate(wo_lines):
                        if idx == len(wo_lines) - 1:
                            new_qty = float_round(remaining, precision_rounding=move_round)
                        else:
                            ratio = 0.0
