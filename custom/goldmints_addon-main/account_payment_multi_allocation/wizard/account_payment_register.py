...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _refresh_allocation_amount(self):
        ...
        if self._has_wht_source_lines():
            # WHT modules own the net-payment amount. Do not overwrite with
            # allocation totals (which represent gross invoice amounts).
            return
        if self._is_fx_amount_context():
            # In FX context, keep amount synced to core installment/default amount
            # so user doesn't need to click "installments" manually after changing currency.
