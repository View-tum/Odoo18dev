...
class SaleOrder(models.Model):
                ...
                if rec.mapped('invoice_ids'):
                    if rec.mapped('invoice_ids'):
                        ...
                        if reconcile_lines:
                            ...
                            payments = self.env['account.payment'].search([
                                '|',
                                ('invoice_ids.id', 'in', reconcile_lines.mapped('credit_move_id').ids),
                                ('invoice_ids.id', 'in', reconcile_lines.mapped('debit_move_id').ids)
                            ])
    ...
    def _sh_unreseve_qty(self,picking_id):
        ...
        for move_line in picking_id.sudo().mapped('move_ids_without_package').mapped('move_line_ids'):
            if move_line.product_id.type in ['service','combo']:
                continue
