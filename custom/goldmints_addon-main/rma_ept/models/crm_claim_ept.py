...
class CrmClaimEpt(models.Model):
    ...
    state = fields.Selection([
        ('draft', 'Draft'), ('approve', 'Approved'),('approve2', 'Approved2'), ('process', 'Processing'),
    ...
    return_picking_count = fields.Integer(compute='_compute_return_picking_count')
    ...
    delivery_picking_count = fields.Integer(compute='_compute_delivery_picking_count')
    refund_invoice_ids = fields.Many2many(ACCOUNT_MOVE, string='Refund Invoices', copy=False)
    refund_invoice_count = fields.Integer(compute='_compute_refund_invoice_count')
    ...
    def _compute_move_product_ids(self):
        ...
        for record in self:
            ...
            if record.picking_id:
                products = record.picking_id.move_ids.mapped('product_id')
                if products:
    ...
    @api.depends('to_return_picking_ids')
    def _compute_delivery_picking_count(self):
        for record in self:
            record.delivery_picking_count = len(record.to_return_picking_ids)

    @api.depends('refund_invoice_ids')
    def _compute_refund_invoice_count(self):
        for record in self:
            record.refund_invoice_count = len(record.refund_invoice_ids)
    ...
    def create_return_picking(self, claim_lines=False):
        ...
        action_res = return_picking_wizard.action_create_returns()
    ...
    def check_claim_line_validate(line):
        ...
        if line.quantity <= 0:
            raise UserError(_("Please set Return Quantity for all products."))
        if not line.rma_reason_id:
            raise UserError(_("Please set Return Reason for all products."))
        if not line.claim_type:
            raise UserError(_("Please set Action for all products."))
    ...
    def act_supplier_invoice_refund_ept(self):
        ...
        invoices = self.refund_invoice_ids
        if len(invoices) > 1:
    ...
    def prepare_and_create_refund_invoice(self, refund_invoice_ids, refund_invoice_ids_rec):
        ...
        for invoice_id, lines in refund_invoice_ids.items():
            ...
            for line in lines:
                ...
                account = accounts.get('income') or accounts.get('income_unaffected') or accounts.get('expense')
                if not account:
                    raise UserError(_("No income account defined for product %s") % product_id.display_name)
    ...
    def create_reverse_move_for_invoice(self, invoice_id):
        ...
        reason_label = self._get_refund_reason_label()
    ...
    def _get_refund_reason_label(self):
        reasons = self.claim_line_ids.mapped('rma_reason_id')
        labels = []
        seen_keys = set()
        for reason in reasons:
            key = reason.code or reason.name
            if not key or key in seen_keys:
                continue
            seen_keys.add(key)
            label = reason.name or reason.code
            if label:
                labels.append(label)
        return ", ".join(labels) if labels else self.name
