...
class CRMClaimLine(models.Model):
    ...
    claim_type = fields.Selection([
        ...
        ('repair', 'Repair')], string="Action", copy=False)
    ...
    def _compute_return_quantity(self):
        ...
        for record in self:
            ...
            if record.claim_id.return_picking_id:
                move_line = record.claim_id.return_picking_id.move_ids.filtered(
                    lambda r, record=record: r.sale_line_id.id == record.move_id.sale_line_id.id
                    and r.product_id.id == record.product_id.id
                    and r.origin_returned_move_id.id == record.move_id.id
                    and r.state != 'cancel'
                )
