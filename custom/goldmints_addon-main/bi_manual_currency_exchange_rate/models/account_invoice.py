...
from odoo.tools import OrderedSet, float_compare, float_is_zero, float_repr, float_round
        ...
        for line in self:
            ...
            if line.purchase_line_id:
                ...
                if purchase_order.purchase_manual_currency_rate_active:
                    ...
                    # Use discounted unit price (not subtotal) to avoid multiplying by qty when splitting serials
                    po_line = line.purchase_line_id

                    # FIX 1: Use unit price correctly considering discounts if any.
                    # Avoid using price_subtotal / product_qty which fails on partial receipts / serial splits.
                    if hasattr(po_line, '_get_discounted_price_unit'):
                        price_unit_base = po_line._get_discounted_price_unit()
                    elif getattr(po_line, 'discount', False):
                        price_unit_base = po_line.price_unit * (1 - (po_line.discount / 100.0))
                    else:
                        price_unit_base = po_line.price_unit

                    manual_rate = purchase_order.purchase_manual_currency_rate
                    company_currency = line.company_id.currency_id

                    if is_inverted_rate:
                        price_unit = price_unit_base * manual_rate
                    else:
                        price_unit = price_unit_base / (manual_rate or 1.0)

                    for val in valuation:
                        # FIX 2: Multiply by val.quantity to get the actual total SVL layer value.
                        # Do not hardcode the layer value to exactly price_unit!
                        correct_value = company_currency.round(price_unit * val.quantity)
                        val.write({
