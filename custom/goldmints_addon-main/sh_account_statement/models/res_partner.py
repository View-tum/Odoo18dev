...
class ResPartner(models.Model):
    ...
    def _compute_statements(self):
        ...
        for rec in self:
            ...
            # ================================================================================
            partner_and_children_ids = [rec.id] + rec.child_ids.ids
            if rec.customer_rank > 0:

                # ------- GET DATA OF INVOICE AND CREDIT NOTE FOR CURRENT PARTNER ----------

                c_moves_statements = self.env['account.move'].search([
                    ('partner_id', 'in', partner_and_children_ids),
                ...
                self._cr.execute(""" select id from account_payment where partner_id in %s and is_reconciled=False and partner_type='customer' and move_id in (select id from account_move where company_id in %s) """,[tuple(partner_and_children_ids),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_invoice' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date,current_date-timedelta(days=30),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_refund' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date,current_date-timedelta(days=30),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_invoice' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=30),current_date-timedelta(days=60),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_refund' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel')  and company_id in %s """,[tuple(partner_and_children_ids),current_date-timedelta(days=30),current_date-timedelta(days=60),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_invoice' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=60),current_date-timedelta(days=90),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_invoice' and invoice_date<=%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=90),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='out_refund' and invoice_date<=%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=90),tuple(company_ids)])
            ...
            if rec.supplier_rank > 0:
                ...
                self._cr.execute(""" select id from account_move where partner_id in %s and move_type in ('in_invoice','in_refund') and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),tuple(company_ids)])
                ...
                self._cr.execute(""" select id from account_payment where partner_id in %s and is_reconciled=False and partner_type='supplier'  and move_id in (select id from account_move where company_id in %s) """,[tuple(partner_and_children_ids),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_invoice' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date,current_date-timedelta(days=30),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_refund' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date,current_date-timedelta(days=30),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_invoice' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date-timedelta(days=30),current_date-timedelta(days=60),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_refund' and invoice_date<=%s and  invoice_date>%s  and state not in ('draft','cancel') and company_id in %s """,[tuple(partner_and_children_ids),current_date-timedelta(days=30),current_date-timedelta(days=60),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_invoice' and invoice_date<=%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=90),tuple(company_ids)])
                ...
                self._cr.execute(""" select id,amount_residual from account_move where partner_id in %s and move_type ='in_refund' and invoice_date<=%s  and state not in ('draft','cancel') and company_id in %s  """,[tuple(partner_and_children_ids),current_date-timedelta(days=90),tuple(company_ids)])
                ...
                if due_bill:
                    ...
                    for bill in due_bill.filtered(lambda i:i.move_type=='in_refund'):
                        ...
                        self.env['sh.vendor.due.statement'].sudo().create(overdue_statement_vals)
    ...
    def action_print_customer_due_statement_xls(self):
        ...
        desired_timezone = self.env.context.get("tz") or self.env.user.tz or "UTC"
        local_time = now_utc.astimezone(pytz.timezone(desired_timezone))
        today_date = local_time.strftime("%d/%m/%Y %H:%M:%S")
    ...
    def action_get_customer_statement(self):
        ...
        if self.customer_rank > 0 and self.start_date and self.end_date:
            ...
            if moves:
                ...
                for move in moves:
                    statement_vals = {
                        "sh_account": move.partner_id.property_account_receivable_id.name,
                        "name": move.name,
                        "currency_id": move.currency_id.id,
    ...
    def action_get_vendor_statement(self):
        ...
        if self.supplier_rank > 0 and self.start_date and self.end_date:
            ...
            partner_list=[self]
            partner_ids=[self.id]
            account_ids=[self.property_account_payable_id.id]
            if self.company_type == 'company':
                if self.child_ids:
                    partner_list.extend(self.child_ids)
                    partner_ids.extend(self.child_ids.ids)
                    account_ids.extend(self.child_ids.mapped('property_account_payable_id').ids)
            ...
            if moves:
                ...
                for move in moves:
                    statement_vals = {
                        "sh_account": move.partner_id.property_account_payable_id.name,
                        "name": move.name,
                        "currency_id": move.currency_id.id,
                ...
                if partner.supplier_rank > 0:
                    ...
                    if not partner.sh_dont_send_statement_auto:
                        if (
                            ...
                            if self.env.company.filter_only_unpaid_bill_and_send_that and not partner.sh_vendor_statement_ids.filtered(lambda x:x.sh_vendor_balance > 0):
                                continue
                    ...
                    if not partner.sh_dont_send_due_statement_auto:
                        if (
                            ...
                            if self.env.company.filter_only_unpaid_bill_and_send_that and not partner.sh_vendor_due_statement_ids.filtered(lambda x:x.sh_due_vendor_balance > 0):
                                continue
