...
class SaleOrder(models.Model):
   ...
   # ---------- Helpers ----------
            ...
            if not order.id:
                ...
                for product, target_qty in free_map.items():
                    ...
                    if ex:
                        ...
                        if need:
                            #2026-02-19 remove [FREE] from vals["name"] to prevent double [FREE]
                            v1 = vals["name"]
                            if v1:
                                v1 = v1.replace("[FREE]", "")
                            #2026-02-19 remove [FREE] from vals["name"] to prevent double [FREE] end
    ...
    def create(self, vals_list):
        ...
        #2026-02-20 add code to remove free item when create
        for order in orders:
            for line in order.order_line:
                if "[FREE]" in line.name:
                    line.unlink()
        #2026-02-20 add code to remove free item when create end
