...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        effective_deductions = self.deduction_ids.filtered(lambda deduct: not deduct.is_open)
        use_multi_deduct = bool(effective_deductions) and (
            self.payment_difference_handling == "reconcile_multi_deduct"
            or len(effective_deductions) > 1
        )
        # payment difference
        if (
            not self.currency_id.is_zero(self.payment_difference)
            and self.payment_difference_handling == "reconcile"
            and payment_vals.get("write_off_line_vals")
        ):
            payment_vals["write_off_line_vals"][0]["analytic_distribution"] = (
                self.analytic_distribution
            )
        # multi deduction
        elif self.payment_difference and use_multi_deduct:
            new_lines = [
                self._prepare_deduct_move_line(deduct)
                for deduct in effective_deductions
            ]
            if "write_off_line_vals" in payment_vals and payment_vals["write_off_line_vals"]:
                payment_vals["write_off_line_vals"].extend(new_lines)
            else:
                payment_vals["write_off_line_vals"] = new_lines
