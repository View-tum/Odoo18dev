        ...
        for line_vals in line_vals_list:
            if all(
                ...
                for update_key in update_keys:
                    if update_key in write_off_line_vals[0]:
    ...
    def _prepare_move_line_default_vals(
        ...
        * Combine all process and return list
        ...
        if not self.is_multi_deduction and write_off_line_vals:
            ...
            if writeoff_lines:
                # If there are multiple write-off lines, pick the first one matching the account (e.g. not the FX line)
                target_writeoff_line = writeoff_lines.filtered(lambda l: l.account_id.id == write_off_line_vals[0].get('account_id'))[:1]
                if not target_writeoff_line:
                     target_writeoff_line = writeoff_lines[:1]

                if not write_off_line_vals[0].get("analytic_distribution", False):
                    write_off_line_vals[0]["analytic_distribution"] = (
                        target_writeoff_line.analytic_distribution
                    )
