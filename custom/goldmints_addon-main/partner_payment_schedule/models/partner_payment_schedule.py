...
class PpsSchedule(models.Model):
    ...
    mode = fields.Selection(
        selection=[
            ("dom", "ระบุวันที่"),
            ("dow", "ระบุวันในสัปดาห์"),
            ("specific", "วันเฉพาะเจาะจง"),
        ],
        required=True,
        default="dom",
    ...
    selected_dates = fields.Text(
        string="วันที่เลือก",
        help="JSON list of selected dates, e.g. ['2026-01-21', '2026-02-04']",
    )
    ...
    selected_dates_count = fields.Integer(
        string="จำนวนวันที่เลือก",
        compute="_compute_selected_dates_count",
        store=True,
    )
    selected_dates_display = fields.Char(
        string="วันที่เลือก",
        compute="_compute_selected_dates_display",
        store=True,
    )

    @api.depends("selected_dates")
    def _compute_selected_dates_display(self):
        import json
        from datetime import datetime
        for rec in self:
            if not rec.selected_dates:
                rec.selected_dates_display = ""
                continue
            try:
                dates = json.loads(rec.selected_dates)
                if not dates:
                    rec.selected_dates_display = ""
                    continue
                ...
                for d_str in sorted(dates):
                    try:
                        ...
                        formatted.append(d_obj.strftime("%d/%m"))
                    except:
                        formatted.append(d_str)
                rec.selected_dates_display = ", ".join(formatted)
            except:
                rec.selected_dates_display = ""

    @api.depends("selected_dates")
    def _compute_selected_dates_count(self):
        import json
        for rec in self:
            if not rec.selected_dates:
                rec.selected_dates_count = 0
                continue
            try:
                dates = json.loads(rec.selected_dates)
                rec.selected_dates_count = len(dates)
            except (ValueError, TypeError, json.JSONDecodeError):
                rec.selected_dates_count = 0

    def action_open_calendar(self):
        return {
            "name": "เลือกวันที่จ่ายเงิน/วางบิล",
            "type": "ir.actions.act_window",
            "res_model": "pps.schedule",
            "res_id": self.id,
            "view_mode": "form",
            "target": "new",
    ...
    @api.constrains("mode", "dom_day_ids", "dom_everyday", "dow_weekday_ids", "selected_dates")
    ...
    def _compute_candidate_local(self, reference: datetime) -> Optional[datetime]:
        if self.mode == "dom":
            ...
            return self._next_local_specific(reference)
        return None

    def _next_local_specific(self, reference: datetime) -> Optional[datetime]:
        import json
        if not self.selected_dates:
            return None
        try:
            date_strs = json.loads(self.selected_dates)
    ...
    def _next_dow_specific_week(self, ref_dt: datetime, weekday_indices: list[int]) -> Optional[datetime]:
        week_no = -1 if self.week_rule == "last" else int(self.week_rule)
        today = ref_dt.date()
    ...
    def _nth_weekday_in_month(y: int, m: int, weekday_idx: int, n: int) -> Optional[date]:
        if n == -1:
            ...
            last_date = date(y, m, last_day_month)
            delta = (last_date.weekday() - weekday_idx) % 7
...
class ResPartner(models.Model):
    ...
    pps_schedule_ids = fields.One2many(
        "pps.schedule",
        "partner_id",
        string="ตารางรับเช็ค (Payment)",
        domain=[("schedule_type", "=", "payment")],
        help="กำหนดรอบรับเช็คสำหรับลูกค้า",
    )
