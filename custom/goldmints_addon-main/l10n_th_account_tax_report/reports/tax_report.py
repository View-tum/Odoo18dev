...
class ThaiTaxReport(models.AbstractModel):
    ...
    # 2026-02-18 add function for get tax invoice type query
    def _get_tax_invoice_type_where_clause(self, so_type):
        if so_type:
    ...
    def _get_tax_invoice_type(self, so_type_id, company):
        if so_type_id:
            so_type = self.env["sale.sequence.type"].sudo().browse(so_type_id.id)
            if so_type.market_scope == "inter":
                return company.inter_journal_id.code  # "INV-E"
            else:
                if not so_type.is_full_tax_invoice:
