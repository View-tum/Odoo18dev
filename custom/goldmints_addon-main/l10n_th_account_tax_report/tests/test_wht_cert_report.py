...
class TestWithholdingTaxReport(TestWithholdingTax):
    ...
    def test_02_wht_button_export_pdf_std(self):
        self.env.user.company_id.wht_report_format = "std"
    ...
    def test_10_wht_button_export_text_file_pnd3_rd_v2(self):
        self.env.user.company_id.wht_report_format = "rd"
        self.wht_report_pnd3_wizard.write(
            {
                "show_cancel": False,
                "rd_sender_id": "0000",
                "rd_sender_nid": "1234567890123",
                "rd_sender_branch": "0000",
                "rd_sender_role": "1",
                "rd_dept_name": "สำนักงานใหญ่",
                "rd_lto": False,
                "rd_branch_type": "V",
                "rd_form_type": "00",
                "rd_submission_seq": "00",
                "rd_form_flag": "1",
                "rd_user_id": "TESTUSER001",
                "rd_sur_amt": 0.0,
                "rd_trans_amt": 0.0,
                "rd_section3": False,
                "rd_section48": False,
        ...
        lines = res_data["text_file_value"].split("\r\n")
        self.assertEqual(len(lines), 2)
        header = lines[0].split("|")
        detail = lines[1].split("|")
        self.assertEqual(len(header), 25)
        self.assertEqual(len(detail), 38)
        self.assertEqual(header[0], "H")
        self.assertEqual(header[5], "PND3")
        self.assertEqual(header[6], "1234567890123")
        self.assertEqual(header[7], "000000")
        self.assertEqual(header[14], str(self.wht_report_pnd3_wizard.date_from.year + 543))
        self.assertEqual(header[15], "V")
        self.assertEqual(header[17], "1")
        self.assertEqual(header[18], "100.00")
        self.assertEqual(header[19], "3.00")
        self.assertEqual(header[20], "0.00")
        self.assertEqual(header[21], "3.00")
        self.assertEqual(header[23], "TESTUSER001")
        self.assertEqual(detail[0], "D")
        self.assertEqual(detail[3], "3012345678901")
        self.assertEqual(detail[4], "0000000000")
        self.assertEqual(detail[10], "100.00")
        self.assertEqual(detail[11], "3.00")
        self.assertNotIn(",", res_data["text_file_value"])
        self.assertEqual(
            self.wht_report_pnd3_wizard._get_report_base_filename(),
            f"PND3_1234567890123_000000_{self.wht_report_pnd3_wizard.date_from.year + 543}_{self.wht_report_pnd3_wizard.date_from.month:02d}_00_00",
        )

    def test_11_wht_button_export_text_file_pnd53_rd_v2_multi_detail(self):
        self.env.user.company_id.wht_report_format = "rd"
        self.wht_report_pnd53_wizard.write(
            {
                "show_cancel": False,
                "rd_sender_id": "0000",
                "rd_sender_nid": "1234567890123",
        ...
        header = lines[0].split("|")
        detail_1 = lines[1].split("|")
        detail_2 = lines[2].split("|")
        self.assertEqual(len(header), 25)
        self.assertEqual(len(detail_1), 38)
        self.assertEqual(len(detail_2), 38)
        self.assertEqual(header[5], "PND53")
        self.assertEqual(header[10], "1")
        self.assertEqual(header[17], "2")
        self.assertEqual(header[18], "1000.00")
        self.assertEqual(header[19], "24.00")
        self.assertEqual(header[20], "10.00")
        self.assertEqual(header[21], "34.00")
        self.assertEqual(header[22], "5.00")
        self.assertEqual(detail_1[10], "100.00")
        self.assertEqual(detail_1[16], "200.00")
        self.assertEqual(detail_1[22], "300.00")
        self.assertEqual(detail_2[10], "400.00")
        self.assertEqual(detail_2[16], "0.00")
        self.assertEqual(detail_2[22], "0.00")
