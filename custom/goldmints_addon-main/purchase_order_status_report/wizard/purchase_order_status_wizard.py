...
class PurchaseOrderStatusReportWizard(models.TransientModel):
    ...
    name = fields.Char(default="รายงานสถานะคำสั่งซื้อ")
    ...
    line_ids = fields.One2many(
        "purchase.order.status.report.wizard.line", 
        "wizard_id", 
    ...
    def button_preview(self):
        self.ensure_one()
        self.line_ids = [(5, 0, 0)] # เคลียร์ข้อมูลเก่า

        report_model = self.env["report.purchase_order_status_report.po_status_report"]
        lines_data = report_model._get_lines(self)
