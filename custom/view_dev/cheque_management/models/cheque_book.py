...
class ChequeBookLines(models.Model):
    ...
    def _compute_state(self):
        for record in self:
            ...
            cheque_in_out_bound_obj = self.env['cheque.inbound.outbound'].search(
                [('cheque_id', '=', record.id)],
                order='id desc',
            )
            # Reuse policy: a voided cheque transaction remains for audit, but the cheque
            # book line becomes available again unless there is a newer active transaction.
            active_cheque = cheque_in_out_bound_obj.filtered(lambda c: c.state != 'cancelled')[:1]
            if active_cheque:
                state = active_cheque.state
                if state == 'draft':
                    ...
                    # For other internal states (confirmed/bank_deposit/transform), keep it unavailable.
                    record.status = 'confirmed'
            elif cheque_in_out_bound_obj:
                record.is_state_draft = True
                record.status = 'draft'
            else:
                record.is_state_draft = True
