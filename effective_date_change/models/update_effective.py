...
class UpdateEffective(models.Model):
    ...
    def read(self, fields=None, load='_classic_read'):
        """
        Override read to update effective date to current time + offset
        when viewing draft/ready pickings.
        """
        if not self.env.context.get('skip_update_effective_date'):
            # Filter records that need update: not done/cancel, and has picking type
            ...
            if to_update:
                ...
                # Threshold to respect immediate user saves (e.g. 2 seconds)
                threshold = timedelta(seconds=2)
                
                for rec in to_update:
                    # Skip if record was just written to (likely user save)
                    ...
                    # Update without triggering recursion or validation loops
                    # Using sudo to ensure update even if user has read-only access but needs to see correct date
    ...
    def button_validate(self):
        for picking in self:
            ...
            if not picking.date_of_transfer:
                # raise ValidationError('The Effective Date is empty') default effective module
                    ...
                    if picking.picking_type_id.code == 'incoming' and int(purchase_order.currency_id.id) != system_default_currency:
                        ...
                        for line in picking.move_ids_without_package:
                            ...
                            if product_id in po_details:
                                ...
                                if line_qty <= po_qty:
                                    ...
                                    if line.product_id.lot_valuated:
                                        unit_cost = total_value / line_qty if line_qty else 0.0
                                    else:
                                        unit_cost = total_value
                                    price_unit[product_id] = unit_cost
                        ...
                        if journal_entry_ids:
                            for journal_entry_id in journal_entry_ids:
                                ...
                                # Create the new sequence prefix
                                new_sequence_prefix = f"{account_move.journal_id.code}/{year}/{month}/"
