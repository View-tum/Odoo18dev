...
class Base(models.AbstractModel):
    ...
    def _load_file_data(self, field_name: str = 'raw', 
        ...
        if not file_type and self._name == "ir.attachment":
    ...
    def _ensure_column_exists(self, fields: list[str] | str, default_value = None):        
        """
            Ensure database columns exist for specified fields.

            This method creates database columns for fields that exist in the model's field
            definitions but are missing from the actual database table. This is particularly
            useful for disabling computed fields during module installation to avoid delay in 
            large table computations.

            Args:
                fields (list[str] | str): Field name(s) to ensure columns exist for.
                             Can be a single field name as string or a list
                             of field names.
                default_value: Default value to set for existing records.

            Note:
                Only creates columns for fields that:
                - Exist in the model's _fields dictionary
                - Don't already have a corresponding database column
                
            Example:
                
            def _auto_init(self):
                self._ensure_column_exists('computed_field_name')
        ...
        if isinstance(fields, str):
            fields = [fields]
        for name in fields:
            if (field := self._fields.get(name)) and not column_exists(self.env.cr, self._table, name):
                field.update_db_column(self, None)
                if default_value is not None:
                    self.env.cr.execute(f"UPDATE {self._table} SET {name} = %s", (default_value,))
            elif not field:
                _logger.warning("Field %s does not exist in model %s", name, self._name)