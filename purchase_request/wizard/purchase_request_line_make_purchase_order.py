...
class PurchaseRequestLineMakePurchaseOrderItem(models.TransientModel):
    ...
    def _check_product_qty(self):
        for rec in self:
            if rec.product_qty > rec.line_id.pending_qty_to_receive:
                raise UserError(
                    _("Quantity to purchase cannot exceed the pending quantity to receive (%s).")
                    % rec.line_id.pending_qty_to_receive
                )

    @api.onchange("product_qty")
    def _onchange_product_qty(self):
        if self.product_qty > self.line_id.pending_qty_to_receive:
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "Quantity to purchase cannot exceed the pending quantity to receive (%s)."
                    )
                    % self.line_id.pending_qty_to_receive,
                }
            }
