...
class PurchaseRequestLine(models.Model):
    ...
    def _compute_qty_to_buy(self):
        for pr in self:
            qty_to_buy = sum(pr.mapped("product_qty")) - \
                sum(pr.mapped("purchased_qty"))
            precision = pr.product_uom_id.rounding or 0.01
            pr.qty_to_buy = float_compare(
                qty_to_buy, 0.0, precision_rounding=precision) > 0
    ...
    @api.depends("purchase_lines", "purchase_lines.state", "purchase_lines.product_qty", "purchase_lines.product_uom")
    ...
    def action_show_details(self):
        self.ensure_one()
        view = self.env.ref(
            "purchase_request.view_purchase_request_line_details")
        return {
