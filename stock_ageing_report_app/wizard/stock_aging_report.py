...
class StockAgingReportWizard(models.TransientModel):
	...
	def get_columns(self, data):
		...
		for i in range(0,5):
			if i == 0:
				col = str(current_period_length) + "-" + str(current_period_length + period_length)
			else:
				col = str(current_period_length+1) + "-" + str(current_period_length + period_length)
			current_period_length += period_length
			column_data.append(col)
		col = "> " + str(current_period_length+1)
		column_data.append(col)
		return column_data
...
# Warehouse
	...
	def _get_date_data(self, datas):
		...
		for i in range(0, 6):
			...
			if i <= 0:
				...
				start_date = (start_date - timedelta(days= datas.get('period_length'))) - timedelta(days=1)
				end_date = (end_date - timedelta(days= datas.get('period_length'))) - timedelta(days=1)
				if isinstance(start_date, datetime):
					start_date = datetime.strftime(start_date, DEFAULT_SERVER_DATE_FORMAT)
				if isinstance(end_date, datetime):
					end_date = datetime.strftime(end_date, DEFAULT_SERVER_DATE_FORMAT)
	...
	def _get_product_info(self, product_id, warehouse_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(qty_done),0)
							from stock_move sm,stock_move_line sml,stock_picking_type spt
							where sm.id=sml.move_id
							and sm.picking_type_id=spt.id
							and sml.product_id=%s
							and sml.state='done'
							and sml.company_id=%s
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s
							and spt.code='outgoing'
							and sm.warehouse_id=%s
							and origin_returned_move_id is null;""",(product_id, company_id, end_date, start_date, warehouse_id,))
	...
	def _get_return_in_qty(self, product_id, warehouse_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(qty_done),0)
							...
							and spt.code='outgoing'
							and sm.warehouse_id=%s
							and origin_returned_move_id is not null;""",(product_id, company_id, end_date, start_date, warehouse_id,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_adjusted_qty(self, product_id, warehouse_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(qty_done),0)
							from stock_move sm,stock_move_line sml,stock_location sl
							where sm.id=sml.move_id
							...
							and sml.company_id=%s
							and usage='inventory'
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s;""",(product_id, company_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
...
# Location
	...
	def _get_product_location_info(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_picking_type spt
							where sm.id=sml.move_id
							and sm.picking_type_id=spt.id
							and sml.product_id=%s
							and sml.state='done'
							and sm.company_id=%s
							and sml.location_id=%s
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s
							and spt.code='outgoing';""",(product_id, company_id, location_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_product_location_in_info(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							...
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s
							and spt.code='incoming';""",(product_id, company_id, location_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_return_location_in_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							...
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s
							and spt.code='outgoing'
							and origin_returned_move_id is not null;""",(product_id, company_id, location_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_internal_location_in_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_picking_type spt
							where sm.id=sml.move_id
							and sm.picking_type_id=spt.id
							...
							and sml.location_dest_id=%s
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s
							and spt.code='internal';""",(product_id, company_id, location_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_internal_location_out_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_picking_type spt
							where sm.id=sml.move_id
							and sm.picking_type_id=spt.id
	...
	def _get_pos_return_location_in_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_picking_type spt
							where sm.id=sml.move_id
							and sm.picking_type_id=spt.id
	...
	def _get_adjusted_location_in_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_location loc
							where sm.id=sml.move_id
							and sml.location_id=loc.id
							and sml.product_id=%s
							and sml.state='done'
							and sm.company_id=%s
							and sml.location_dest_id=%s
							and usage='inventory'
							and ((sml.date at time zone 'utc' )at time zone 'asia/bangkok')::date between %s and %s;""",(product_id, company_id, location_id, end_date, start_date,))
		result = self.env.cr.fetchall()
		if result:
			return result[0][0]
		else:
	...
	def _get_adjusted_location_out_qty(self, product_id, location_id, start_date, end_date, company_id):
		self.env.cr.execute("""select COALESCE(sum(sml.quantity),0)
							from stock_move sm,stock_move_line sml,stock_location loc
							where sm.id=sml.move_id
							and sml.location_dest_id=loc.id
							and sml.product_id=%s
							and sml.state='done'
							and sm.company_id=%s
							and sml.location_id=%s
		...
		if location:
			...
			for product_id  in product_ids:
				...
				for date_data in self._get_date_data(data):
					...
					internal_in_qty = self._get_internal_location_in_qty(product_id.id, location_id, start_date, end_date, company_id)
					internal_out_qty = self._get_internal_location_out_qty(product_id.id, location_id, start_date, end_date, company_id)
					pos_return_in_qty = self._get_pos_return_location_in_qty(product_id.id, location_id, start_date, end_date, company_id)
					adjusted_in_qty = self._get_adjusted_location_in_qty(product_id.id, location_id, start_date, end_date, company_id)
					adjusted_out_qty = self._get_adjusted_location_out_qty(product_id.id, location_id, start_date, end_date, company_id)
					qty_on_hand = (received_qty + adjusted_in_qty + return_in_qty + internal_in_qty + pos_return_in_qty) - (delivered_qty + adjusted_out_qty + return_out_qty + internal_out_qty)
					qty_hand_key = col + str(counter)
					value.update({ qty_hand_key : qty_on_hand })
					counter += 1
				product_data.append(value)
	...
	def print_excel_report(self):
		...
		output.close()
		buf = base64.b64encode(generated_file)
		self.write({'document': buf})
		...
		if self.document:
			...
			return {
				...
				'url': 'web/content/?model=stock.aging.report.wizard&download=true&field=excel_file&id=%s&filename=%s' % (
					active_id, self.file),
				'target': 'new',
			}
