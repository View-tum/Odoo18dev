...
class AdvanceCashLog(models.Model):
    ...
    is_return_bank_draft_method = fields.Boolean(
        compute="_compute_is_return_bank_draft_method"
    )
    bank_draft_number = fields.Char(string="Bank Draft Number", copy=False)
    bank_draft_bank_id = fields.Many2one("res.bank", string="Issue Bank", copy=False)
    bank_draft_branch = fields.Char(string="Bank Branch", copy=False)
    bank_draft_date = fields.Date(string="Bank Draft Date", copy=False)
    bank_draft_form_id = fields.Many2one(
        "dynamic.cheque", string="Custom Cheque Form", copy=False
    )

    @api.depends("transaction_type", "return_payment_method_line_id")
    def _compute_is_return_bank_draft_method(self):
        for rec in self:
            line = rec.return_payment_method_line_id
            rec.is_return_bank_draft_method = bool(
                rec.transaction_type == "return"
                and line
                and (
                    line.code == "bank_draft"
                    or line.is_bank_draft_incoming_line
                )
            )

    @api.onchange("return_payment_journal_id")
    def _onchange_return_payment_journal_id(self):
        self.return_payment_method_line_id = False
        if self.return_payment_journal_id:
            self.return_payment_method_line_id = self.env[
                "account.payment.method.line"
            ].search(
                [
                    ("journal_id", "=", self.return_payment_journal_id.id),
                    ("payment_type", "=", "inbound"),
                ],
                limit=1,
            )

    def action_open_bank_draft(self):
        self.ensure_one()
        return {
            "name": "Bank Draft",
    ...
    def _is_return_bank_draft_payment(self, payment_data):
        payment_method_line = payment_data.get("payment_method_line_id")
        return bool(
            self.transaction_type == "return"
            and payment_data.get("is_bank_draft_method")
            and payment_method_line
            and (
                payment_method_line.code == "bank_draft"
                or payment_method_line.is_bank_draft_incoming_line
            )
        )

    def _get_return_payment_data_from_record(self):
        if not self.return_payment_journal_id or not self.return_payment_method_line_id:
            return False
        return {
            "payment_journal_id": self.return_payment_journal_id,
            "payment_method_line_id": self.return_payment_method_line_id,
            "is_cheque_method": False,
            "is_bank_draft_method": self.is_return_bank_draft_method,
    ...
    def _prepare_return_bank_draft_vals(self, payment_data, move):
        partner = self.employee_id.work_contact_id
        draft_number = payment_data.get("bank_draft_number")
        bank = payment_data.get("bank_draft_bank_id")
        branch = payment_data.get("bank_draft_branch")
        draft_date = payment_data.get("bank_draft_date")
        payment_journal = payment_data.get("payment_journal_id")
        payment_method_line = payment_data.get("payment_method_line_id")
        dynamic_cheque = payment_data.get("bank_draft_form_id")

        if not draft_number:
            raise UserError("Bank Draft Number is required.")
        if not bank:
            raise UserError("Issue Bank is required.")
        if not branch:
            raise UserError("Bank Branch is required.")
        if not draft_date:
            raise UserError("Bank Draft Date is required.")

        duplicate = self.env["cheque.inbound.outbound"].search(
            [
        ...
        if duplicate:
            raise UserError(f"Bank Draft Number '{draft_number}' already exists.")
        ...
        return {
            ...
            "cheque_date": draft_date,
            "cheque_received_date": payment_data.get("payment_date") or self.date,
            "payment_ref": self.name,
            "cheque_bank_id": bank.id,
            "cheque_bank_branch": branch,
            "cheque_journal_entry_id": move.id if move else False,
            "dynamic_io_cheque_id": dynamic_cheque.id if dynamic_cheque else False,
            "memo": self.name + (f" - {self.description}" if self.description else ""),
            "company_id": self.company_id.id,
            "advance_cash_id": self.id,
        }

    def _create_return_bank_draft(self, payment_data, move=False):
        vals = self._prepare_return_bank_draft_vals(payment_data, move)
        draft = self.env["cheque.inbound.outbound"].create(vals)
        self.bank_draft_id = draft.id
        return draft

    def _sync_return_bank_draft_with_move(self, payment_data, move):
        if self.bank_draft_id:
            self.bank_draft_id.write(
                {
                    "amount": self.amount,
                    "date": payment_data.get("payment_date") or self.date,
        ...
        return self._create_return_bank_draft(payment_data, move)

    def _ensure_return_bank_draft_before_approval(self):
        self.ensure_one()
        if self.transaction_type != "return" or not self.is_return_bank_draft_method:
            return False
        payment_data = self._get_return_payment_data_from_record()
        if not payment_data:
            raise UserError("Return Payment Journal and Payment Method are required.")
        if self.bank_draft_id:
            return self.bank_draft_id
        return self._create_return_bank_draft(payment_data)
    ...
    def action_approve(self):
        for rec in self:
            ...
            rec._ensure_return_bank_draft_before_approval()
            rec.write(
                {
                    "state": "approved",
        ...
        if self.transaction_type in ("payout", "return"):
            if not payment_data:
                if self.transaction_type == "return":
                    payment_data = self._get_return_payment_data_from_record()
                if payment_data:
                    return self.action_confirm(payment_data=payment_data)
                return {
            ...
            if self._is_return_bank_draft_payment(payment_data):
                self._sync_return_bank_draft_with_move(payment_data, move)
