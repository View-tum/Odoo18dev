...
class MrpProduction(models.Model):
    ...
    def action_open_manual_merge_internal_transfers(self):
        ...
        pickings = (moves.mapped("picking_id") | self.mapped("picking_ids")).filtered(
    ...
    def _action_cancel(self):
        related_pickings = self._get_related_mo_internal_transfer_pickings()
        result = super(
            MrpProduction,
            self.with_context(canceling_mrp_production_ids=self.ids),
        )._action_cancel()
        related_pickings._sync_shared_mo_internal_transfer_quantities()
        return result

    def _get_related_mo_internal_transfer_pickings(self):
    ...
    def _get_purchase_orders(self):
        ...
        return purchase_orders | self._get_purchase_orders_from_origin()

    def _get_purchase_orders_from_origin(self):
        self.ensure_one()
        if not self.name:
        ...
        return purchase_orders.filtered(
            lambda order: self.name in self._get_purchase_origin_tokens(order.origin)
        )

    def _get_purchase_origin_tokens(self, origin):
        return [token.strip() for token in (origin or "").split(",") if token.strip()]
