...
class TestMobileMixedPayment(TransactionCase):
    ...
    def test_sale_order_receive_payment_action_opens_register_payment(self):
        wizard = self._new_wizard(
    ...
    def test_sale_order_receive_payment_action_can_settle_customer_credit_note(self):
        wizard = self._new_wizard(
            1000.0, [{"payment_type": "bank", "amount": 1000.0}]
        )
        wizard.action_create_invoice_mobile()
        ...
        method_line = wizard._get_payment_method_line_for_journal(self.bank_journal, "bank")
        ...
        credit_line = register.same_side_credit_note_line_ids.filtered(
            lambda line: line.move_id == credit_note
        )
        self.assertTrue(credit_line)

        credit_line.is_selected = True
        register._onchange_same_side_credit_note_line_ids()
        self.assertAlmostEqual(register.amount, 750.0)

        result = register.action_create_payments()
