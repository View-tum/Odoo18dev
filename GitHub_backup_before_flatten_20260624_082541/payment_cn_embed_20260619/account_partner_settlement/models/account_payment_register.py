...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_amount(self):
        ...
        for wizard in self:
            ...
            if not wizard.cross_settlement_amount:
                continue
            wizard.amount = wizard.currency_id.round(max(wizard.amount - wizard._get_cross_payment_offset_amount(), 0.0))
    ...
    def _get_cross_adjusted_difference(self, base_difference):
        self.ensure_one()
        effective_cross_payment_offset = self._get_cross_payment_offset_amount()
        if not effective_cross_payment_offset:
            return base_difference
        return self.currency_id.round(max(base_difference - effective_cross_payment_offset, 0.0))
    ...
    def _append_same_side_credit_note_payment_lines(self, payment_vals, batch_lines):
        self.ensure_one()
        same_side_credit_note_vals = self._prepare_same_side_credit_note_payment_line_vals(batch_lines)
    ...
    def _apply_cross_settlement_selection(self):
        ...
        base_cash_amount = self.currency_id.round(
            self.amount + self._get_cross_payment_offset_amount()
        )
        ...
        cash_amount = self.currency_id.round(max(base_cash_amount - self._get_cross_payment_offset_amount(), 0.0))
    ...
    def action_create_payments(self):
        ...
        if self._has_same_side_credit_note_lines(self.line_ids):
