...
class AccountPartnerSettlement(models.Model):
    ...
    wht_cert_count = fields.Integer(string="WHT Certs", compute="_compute_wht_cert_count")
    ...
    @api.depends("settlement_move_id.wht_cert_ids")
    def _compute_wht_cert_count(self):
        for settlement in self:
            settlement.wht_cert_count = len(settlement.settlement_move_id.wht_cert_ids)
    ...
    def action_post_settlement(self):
        for settlement in self:
            ...
            wht_total = settlement.currency_id.round(sum(item["amount"] for item in wht_items))
            expected_credit_total = settlement.currency_id.round(debit_total - wht_total)
            if settlement.currency_id.compare_amounts(credit_total, expected_credit_total) != 0:
                raise UserError(
                    _(
                        "Selected invoice settlement amount must equal vendor bill amount after withholding tax."
                    )
                )
            ...
            for item in wht_items:
                move_line_commands.append(
                    Command.create(
                        {
                            "partner_id": item["partner"].id,
                            "account_id": item["account"].id,
                            "name": "Settlement WHT Bill %s" % item["move"].name,
                            "debit": 0.0,
                            "credit": settlement.currency_id.round(item["amount"]),
    ...
    def action_view_wht_certs(self):
        self.ensure_one()
        certs = self.settlement_move_id.wht_cert_ids
        if not certs:
            raise UserError(_("No WHT Certificates found for this settlement."))
        return {
