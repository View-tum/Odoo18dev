...
class PurchaseOrder(models.Model):
    ...
    total_before_discount = fields.Float('มูลค่าก่อนหักส่วนลด', compute='_compute_total_before_discount', store=True,
                                         readonly=True)
    discount_total_line = fields.Float('รวมส่วนลดรายการ', compute='_compute_discount_total_line', store=True,
                                       readonly=True)
    total_after_discount = fields.Float('มูลค่าหลังหักส่วนลดรายการ (เฉพาะสินค้าที่ไม่มีส่วนลดพิเศษ)',
...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_discount_total(self):
        for line in self:
            if line.product_id.product_tmpl_id.discount == False:
                line.discount_total = line.discount_per_unit * line.product_qty
            else:
    ...
    def _compute_total_before_discount_line(self):
        for line in self:
            if line.product_id.product_tmpl_id.discount == False:
                line.total_before_discount_line = line.product_qty * line.price_unit
            else:
