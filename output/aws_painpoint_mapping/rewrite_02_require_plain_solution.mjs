...
const renderPath = path.join(outputDir, "02_Require_plain_solution_preview.png");
const today = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
const backupPath = path.join(outputDir, `02_Require_before_plain_solution_${today}.xlsx`);
...
await fs.writeFile(path.join(outputDir, "02_Require_before_plain_solution_preview.png"), new Uint8Array(await beforePreview.arrayBuffer()));

for (const [row, solution] of solutions.entries()) {
  sheet.getRange(`D${row}`).values = [[solution]];
}
