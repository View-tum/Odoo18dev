...
const renderPath = path.join(outputDir, "02_Require_final_rebuilt_preview.png");
const today = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
const backupPath = path.join(outputDir, `02_Require_before_final_rebuild_${today}.xlsx`);
...
for (const [row, solution] of solutions.entries()) {
  const cell = sheet.getRange(`D${row}`);
  cell.values = [[solution]];
  workbook.comments.addThread({ cell }, "Note: ข้อความ Solution นี้เขียนเป็นภาษาธุรกิจเพื่ออธิบายว่า Odoo ช่วยตอบโจทย์ requirement ของ AMS อย่างไร รายละเอียด Standard/Custom และ manday ดูใน AWS_Present.xlsx");
}

sheet.getRange("D1:D120").format.wrapText = true;
sheet.getRange("D1:D120").format.columnWidth = 74;
