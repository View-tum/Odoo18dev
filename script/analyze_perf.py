...
base_dir = r'C:\365_project\TheCool18e\Dev\custom\goldmints_addon-main'
    ...
    def visit_Call(self, node):
        if self.in_loop:
            if isinstance(node.func, ast.Attribute):
                ...
                # Check for common Odoo ORM methods that hit the DB
                if func_name in ['search', 'search_count', 'search_read', 'create']:
                    # It's a method call. Is it on self or env?
                    # This is a heuristic. It might have false positives.
                    self.issues.append({
                        'line': node.lineno,
                        'method': func_name,
    ...
    for file in files:
        if file.endswith('.py'):
            ...
            try:
                ...
                pass # Ignore files with syntax errors (e.g. py2)
...
# Print top 15 files with the most issues
for rel_path, issues in sorted_files[:15]:
    print(f'File: {rel_path} ({len(issues)} issues)')
    for issue in issues[:3]: # Print up to 3 examples per file
